...
function generate_image_header() {
	...
	local ARM_HPSRAM=`grep "CONFIG_HPSRAM=" ${ARM_DEFCONFIG} > /dev/null && echo 1 || echo 0`
	local ARM_PSRAM_LOAD_ADDR=`grep "CONFIG_PSRAM_START_ADDRESS=" ${ARM_DEFCONFIG} | awk -F= '{print $NF}'`
	local ARM_HPSRAM_LOAD_ADDR=`grep "CONFIG_HPSRAM_START_ADDRESS=" ${ARM_DEFCONFIG} | awk -F= '{print $NF}'`
	...
	CC_SYMBOLS+="-DARM_HPSRAM=${ARM_HPSRAM} "
	CC_SYMBOLS+="-DARM_PSRAM_LOAD_ADDR=${ARM_PSRAM_LOAD_ADDR} "
	CC_SYMBOLS+="-DARM_HPSRAM_LOAD_ADDR=${ARM_HPSRAM_LOAD_ADDR} "
...
function mlib()
{
    ...
    if [ $# != 2  ] ; then
        ...
        echo " e.g : mlib build/r128s3_pro_rgb_c906/drivers/rtos-hal/hal/source/nand_flash/lib libfreertos-nand-sun20iw2-c906.a"
        return 1;
    fi
    ...
    [ $? -eq 0 ] && echo -e "\033[31m make $(gettop)/lichee/rtos/${1}/${2} successfully \033[0m"
    cd -
}
