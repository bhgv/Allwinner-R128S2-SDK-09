...
board/common/bin/rtos_hpsram_arm_${PACK_CHIP}.fex:image/hpsram_arm.fex
...
board/common/bin/rtos_hpsram_riscv_${PACK_CHIP}.fex:image/rtos_hriscv.fex
...
board/${PACK_PROJECT_PATH}/bin/rtos_hpsram_arm_${PACK_CHIP}.fex:image/hpsram_arm.fex
...
board/${PACK_PROJECT_PATH}/bin/rtos_hpsram_riscv_${PACK_CHIP}.fex:image/rtos_hriscv.fex
...
# and add a partition to sys_partition.fex like this:
# [partition]
#	name         = reserve
#	size         = 1024
#	downloadfile = "reserve.fex"
#	user_type    = 0x8000
function make_rtos_reserve_image_for_nand()
{
	cd ${ROOT_DIR}/${PACK_PROJECT_PATH}/image/
	local RES_PART_NAME=reserve
	cp $1 sys_partition_tmp_res.fex

	printf "handle partition ${RES_PART_NAME}\n"
	sed -i '/^[ \t]*downloadfile/d' sys_partition_tmp_res.fex
	/bin/busybox unix2dos sys_partition_tmp_res.fex
	if [ "x${PACK_HOSTOS}" == "xlinux" ] ; then
		${SCRIPT} sys_partition_tmp_res.fex  > /dev/null
	elif [ "x${PACK_HOSTOS}" == "xwin" ] ; then
		${SCRIPT} sys_partition_tmp_res.bin sys_partition_tmp_res.fex > /dev/null
	fi

	${UPDATE_MBR} sys_partition_tmp_res.bin 1 sunxi_mbr_tmp_res.fex > /dev/null

	local RES_PART_DOWNLOAD_FILE=reserve.fex
	local RES_PART_FILE_PATH=${PACK_TOPDIR}/board/${PACK_PROJECT_PATH}/data/${RES_PART_NAME}
	if [ "x${PACK_HOSTOS}" == "xlinux" ] ; then
		local RES_PART_SIZE_IN_SECTOR=`parser_mbr sunxi_mbr_tmp_res.fex get_size_by_name ${RES_PART_NAME}`
	elif [ "x${PACK_HOSTOS}" == "xwin" ] ; then
		local RES_PART_SIZE_IN_SECTOR=`sed -n "/${RES_PART_NAME}/{N;p}" $1 | awk '$1 !~ /^;/ {print}' | awk '$0~"size"{print $3}'`
	fi

	if [ -d ${RES_PART_FILE_PATH} -a x${RES_PART_SIZE_IN_SECTOR} != x'' ] ; then
		local RES_FILE_SIZE_IN_K=`du --summarize "${RES_PART_FILE_PATH}" | awk '{print $1}'`
		let RES_PART_SIZE_IN_K=${RES_PART_SIZE_IN_SECTOR}/2
		printf "file size kb: ${RES_FILE_SIZE_IN_K}\n"
		printf "partition size kb: ${RES_PART_SIZE_IN_K}\n"
		if [ ${RES_PART_SIZE_IN_K} -le ${RES_FILE_SIZE_IN_K} ] ; then
			printf "file size is larger than partition size, please check your configuration\n"
			printf "please enlarge size of ${RES_PART_NAME} in sys_partition or remove some files in $RES_PART_FILE_PATH\n"
			exit -1
		fi
		let RES_PART_SIZE_IN_BYTE=${RES_PART_SIZE_IN_SECTOR}*512
		echo "RES_PART_DOWNLOAD_FILE = ${ROOT_DIR}/${PACK_PROJECT_PATH}/image/${RES_PART_DOWNLOAD_FILE} size:${RES_PART_SIZE_IN_BYTE} bytes"
		rm -f ${ROOT_DIR}/${PACK_PROJECT_PATH}/image/${RES_PART_DOWNLOAD_FILE}
		mklittlefs ${RES_PART_SIZE_IN_BYTE} ${RES_PART_FILE_PATH} ${RES_PART_DOWNLOAD_FILE}
	else
		printf "no wifi reserve partitions\n"
	fi
	cd - > /dev/null
}
...
# add a partition to sys_partition.fex/sys_partition_nor.fex like this:
# [partition]
#	name         = user-res
#	size         = 1024
#	downloadfile = "user-res.fex"
#	user_type    = 0x8000
function make_user_res()
{
	printf "make user resource for : $1\n"
	cd ${ROOT_DIR}/${PACK_PROJECT_PATH}/image/
	local USER_PART_NAME=user-res
	cp $1 sys_partition_tmp_user.fex

	printf "handle partition ${USER_PART_NAME}\n"
	sed -i '/^[ \t]*downloadfile/d' sys_partition_tmp_user.fex
	/bin/busybox unix2dos sys_partition_tmp_user.fex
	if [ "x${PACK_HOSTOS}" == "xlinux" ] ; then
		${SCRIPT} sys_partition_tmp_user.fex  > /dev/null
	elif [ "x${PACK_HOSTOS}" == "xwin" ] ; then
		${SCRIPT} sys_partition_tmp_user.bin sys_partition_tmp_user.fex > /dev/null
	fi

	${UPDATE_MBR} sys_partition_tmp_user.bin 1 sunxi_mbr_tmp_user.fex > /dev/null

	local USER_PART_DOWNLOAD_FILE=user-res.fex
	local USER_PART_FILE_PATH=${PACK_TOPDIR}/board/${PACK_PROJECT_PATH}/data/${USER_PART_NAME}
	if [ "x${PACK_HOSTOS}" == "xlinux" ] ; then
		local USER_PART_SIZE_IN_SECTOR=`parser_mbr sunxi_mbr_tmp_user.fex get_size_by_name ${USER_PART_NAME}`
	elif [ "x${PACK_HOSTOS}" == "xwin" ] ; then
		local USER_PART_SIZE_IN_SECTOR=`sed -n "/${USER_PART_NAME}/{N;p}" $1 | awk '$1 !~ /^;/ {print}' | awk '$0~"size"{print $3}'`
	fi

	if [ -d ${USER_PART_FILE_PATH} -a  x${USER_PART_SIZE_IN_SECTOR} != x'0' ]; then
		local USER_FILE_SIZE_IN_K=`du --summarize "${USER_PART_FILE_PATH}" | awk '{print $1}'`
		let USER_PART_SIZE_IN_K=${USER_PART_SIZE_IN_SECTOR}/2
		printf "file size kb: ${USER_FILE_SIZE_IN_K}\n"
		printf "partition size kb: ${USER_PART_SIZE_IN_K}\n"
		if [ ${USER_PART_SIZE_IN_K} -le ${USER_FILE_SIZE_IN_K} ]; then
			printf "file size is larger than partition size, please check your configuration\n"
			printf "please enlarge size of ${USER_PART_NAME} in sys_partition or remove some files in $USER_PART_FILE_PATH\n"
			exit -1
		fi
		let USER_PART_SIZE_IN_BYTE=${USER_PART_SIZE_IN_SECTOR}*512
		echo "USER_PART_DOWNLOAD_FILE = ${ROOT_DIR}/image/${USER_PART_DOWNLOAD_FILE} size:${USER_PART_SIZE_IN_BYTE} bytes"
		rm -f ${ROOT_DIR}/${PACK_PROJECT_PATH}/image/${USER_PART_DOWNLOAD_FILE}
		mklittlefs ${USER_PART_SIZE_IN_BYTE} ${USER_PART_FILE_PATH} ${USER_PART_DOWNLOAD_FILE}
	else
...
function do_prepare()
{
	...
	if [ -e ${ROOT_DIR}/${PACK_PROJECT_PATH}/image/sys_partition_nor.fex ];then
		make_user_res ${ROOT_DIR}/${PACK_PROJECT_PATH}/image/sys_partition_nor.fex
	else
		make_user_res ${ROOT_DIR}/${PACK_PROJECT_PATH}/image/sys_partition.fex
		make_rtos_reserve_image_for_nand ${ROOT_DIR}/${PACK_PROJECT_PATH}/image/sys_partition.fex
	fi
}
