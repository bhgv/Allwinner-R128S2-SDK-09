...
#define SUNXI_PRCM_EXT_LDO_CTRL		(SUNXI_GPRCM_BASE + 0x24)
#endif
...
//set ext_ldo voltage to 2.7V
#ifdef CONFIG_ARCH_SUN20IW2
	reg_val = readl(SUNXI_PRCM_EXT_LDO_CTRL);
	reg_val &= ~(0x2 << 4);
	reg_val |= 0x20;
	writel(reg_val, SUNXI_PRCM_EXT_LDO_CTRL);
	udelay(200);
#endif
...
static void _set_cfg_flg(unsigned int efuse_cfg_base, unsigned int bit_offset)
{
	if (bit_offset > 31) {
		pr_err("err:bit_offset more than 32 bits\n");
	}
...
#ifdef CONFIG_MACH_SUN20IW2
#ifdef CONFIG_SUNXI_BOOT_SELECT
int sunxi_set_boot_select(void)
{
	int ret = readl(SID_EFUSE + EFUSE_SYSTEM) & 0x01;
	if (ret) {
		printf("already burn boot select bit! skip.\n");
		return 0;
	}

	// set DCDC_VOL to 2.5V
	writel(((readl(SUNXI_PRCM_BASE+0x4) & (~(0x3f<<12))) | (0x3c<<12)), (SUNXI_PRCM_BASE + 0x4));

	ret = uni_burn_key(EFUSE_SYSTEM, (0x1 << 0));
	if (ret) {
		printf("set boot select bit failed!\n");
	} else {
		printf("set boot select bit ok!\n");
	}

	// set DCDC_VOL to default 1.8V
	writel(((readl(SUNXI_PRCM_BASE+0x4) & (~(0x3f<<12))) | (0x20<<12)), (SUNXI_PRCM_BASE + 0x4));
