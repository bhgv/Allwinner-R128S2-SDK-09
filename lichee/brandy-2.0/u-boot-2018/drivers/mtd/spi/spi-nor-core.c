...
#define CONFIG_SUNXI_INDIVIDUAL_LOCK
#define CONFIG_SUNXI_LOCK
...
#if 0 /* winbond can protect in sector/block unit, so, drop pt_mode */
struct nor_protection winbond_protection[] = {
    { .boundary = 1, .bp = 0, .flag = 0 },
    { .boundary = BP_SZ_256K, .bp = BIT(0), .flag = SET_TB },
    { .boundary = BP_SZ_512K, .bp = BIT(1), .flag = SET_TB },
    { .boundary = BP_SZ_1M, .bp = BIT(1) | BIT(0), .flag = SET_TB },
    { .boundary = BP_SZ_2M, .bp = BIT(2), .flag = SET_TB },
    { .boundary = BP_SZ_4M, .bp = BIT(2) | BIT(0), .flag = SET_TB },
    { .boundary = BP_SZ_8M, .bp = BIT(2) | BIT(1), .flag = SET_TB },
    { .boundary = BP_SZ_12M, .bp = BIT(2) | BIT(0), .flag = SET_CMP },
    { .boundary = BP_SZ_14M, .bp = BIT(2), .flag = SET_CMP },
    { .boundary = BP_SZ_15M, .bp = BIT(1) | BIT(0), .flag = SET_CMP },
    { .boundary = BP_SZ_15872K, .bp = BIT(1), .flag = SET_CMP },
    { .boundary = BP_SZ_16128K, .bp = BIT(0), .flag = SET_CMP },
    { .boundary = BP_SZ_16M, .bp = BIT(2) | BIT(1) | BIT(0), .flag = SET_TB },
};
#endif
...
static int spi_nor_read_addr_reg(struct spi_nor *nor, u8 code, loff_t addr, u8 *val, int len)
{
	struct spi_mem_op op = SPI_MEM_OP(SPI_MEM_OP_CMD(code, 1),
...
#if defined(CONFIG_SPI_FLASH_XT)
/*
...
static int read_sr1(struct spi_nor *nor)
{
	int ret;
	u8 val;

	ret = nor->read_reg(nor, SPINOR_OP_RDSR1, &val, 1);
	if (ret < 0) {
		pr_debug("error %d reading SR1\n", (int)ret);
		return ret;
	}
...
/*
 * Read the status register-3, returning its value in the location
 * Return the status register value.
 * Returns negative if error occurred.
 */
...
/*
 * Write status register-3 1 byte
 * Returns negative if error occurred.
 */
...
static int enter_otp_mode(struct spi_nor *nor)
{
	return nor->write_reg(nor, SPINOR_OP_EN_OTP, NULL, 0);
}

static int exit_otp_mode(struct spi_nor *nor)
{
	return nor->write_reg(nor, SPINOR_OP_EX_OTP, NULL, 0);
}

static int write_otp_sr(struct spi_nor *nor, u8 val)
{
	int ret;
	ret = enter_otp_mode(nor);
	if (ret)
		return ret;
...
extern int sunxi_nor_erase_chip_flag;
#endif
...
#ifdef CONFIG_SUNXI_NOR_SPRITE_TIME_OPTIMIZE
		if (sunxi_nor_erase_chip_flag == 1) {
			ret = spi_nor_erase_chip(nor);
			if (ret)
				goto erase_err;

			sunxi_nor_erase_chip_flag = 0;
			/* clear len to end while loop */
			len = 0;
		} else
#endif
		{
...
#if 0
static int read_spb(struct spi_nor *nor)
{
	int ret;
	u8 val;

	ret = nor->read_reg(nor, SPINOR_OP_RDSPB, &val, 1);
	if (ret < 0) {
		pr_debug("error %d reading spb\n", (int)ret);
		return ret;
	}
...
static int read_scur(struct spi_nor *nor)
{
	...
	if (ret < 0) {
		pr_debug("error %d reading security\n", (int)ret);
		return ret;
	}
...
{
	nor->cmd_buf[0] = val;
	return nor->write_reg(nor, SPINOR_OP_WRSCUR, nor->cmd_buf, 1);
}
#endif

static int macronix_set_wpsel(struct spi_nor *nor)
{
	return nor->write_reg(nor, SPINOR_OP_WPSEL, NULL, 0);
}

#if 0
static int macronix_erase_spb(struct spi_nor *nor)
{
	return nor->write_reg(nor, SPINOR_OP_ESSPB, NULL, 0);
}
#endif

static int macronix_config_individual_mode(struct spi_nor *nor, int mode)
{
	int wps = mode;
	int ret = 0, scur_old, scur_new;

	scur_old = read_scur(nor);
	if (scur_old < 0)
		return scur_old;


	scur_new = scur_old;
	scur_new = wps ? (scur_new | SCUR_WPSEL_MX) : (scur_new & ~SCUR_WPSEL_MX);

	if (scur_old != scur_new) {
		if (wps == 0) {
			ret = -1; /* WPSEL is otp */
		} else {
			write_enable(nor);
			ret = macronix_set_wpsel(nor);
			/* ret = write_scur(nor, scur_new & 0xff); */
			spi_nor_wait_till_ready(nor);

#if 0
			write_enable(nor);
			ret = macronix_erase_spb(nor);
			spi_nor_wait_till_ready(nor);
#endif

		}
	}
	printf("scur:0x%x --> 0x%x\n", scur_old, scur_new);
...
#if defined(CONFIG_SPI_FLASH_XT)
static int write_sr_sr1(struct spi_nor *nor, u8 *sr) {
	return write_sr_cr(nor, sr);
}
#endif
...
#if defined(CONFIG_SUNXI_LOCK)
static void sunxi_get_locked_range(struct spi_nor *nor, u8 sr, u8 cr, loff_t *ofs,
				 uint64_t *len)
{
	/* struct mtd_info *mtd = &nor->mtd; */
	u8 mask, shift;
	...
	dev_info(nor->dev, "ofs:0x%llx,len=0x%lx flag:0x%x sr:0x%x cr:0x%x\n",
 ...
 * @locked is false); 0 otherwise
 */
static int sunxi_check_lock_status_sr_cr(struct spi_nor *nor, loff_t ofs, u64 len,
...
static int sunxi_is_locked_sr_cr(struct spi_nor *nor, loff_t ofs, uint64_t len,
			    u8 sr, u8 cr)
{
	return sunxi_check_lock_status_sr_cr(nor, ofs, len, sr, cr, true);
}

static int sunxi_is_unlocked_sr_cr(struct spi_nor *nor, loff_t ofs, uint64_t len,
			    u8 sr, u8 cr)
{
	return sunxi_check_lock_status_sr_cr(nor, ofs, len, sr, cr, false);
}

static int sunxi_handle_lock(struct spi_nor *nor, loff_t ofs, uint64_t len, int lock)
{
	struct mtd_info *mtd = &nor->mtd;
	...
	u8 mask, shift, val;
	loff_t lock_len;
	bool can_be_top = false, can_be_bottom = true; /* hardcode now, sunxi only use bottom */
	bool use_bottom;
	...
	if (lock) {
		/* If nothing in our range is unlocked, we don't need to do anything */
		if (sunxi_is_locked_sr_cr(nor, ofs, len, status_old, config_old)) {
			printf("already locked\n");
			return 0;
		}
	} else {
		/* If nothing in our range is locked, we don't need to do anything */
		if (sunxi_is_unlocked_sr_cr(nor, ofs, len, status_old, config_old)) {
	...
	if (lock) {
		...
		/* If anything below us is unlocked, we can't use 'bottom' protection */
		if (!sunxi_is_locked_sr_cr(nor, 0, ofs, status_old, config_old)) {
			printf("can not be bottom\n");
			can_be_bottom = false;
		}

		/* If anything above us is unlocked, we can't use 'top' protection */
		if (!sunxi_is_locked_sr_cr(nor, ofs + len, mtd->size - (ofs + len),
					status_old, config_old)) {
			printf("can not be top\n");
			can_be_top = false;
		}
	} else {
		/* If nothing in our range is locked, we don't need to do anything */
		...
		/* If anything below us is locked, we can't use 'top' protection */
		if (!sunxi_is_unlocked_sr_cr(nor, 0, ofs, status_old, config_old))
			can_be_top = false;

		/* If anything above us is locked, we can't use 'bottom' protection */
		if (!sunxi_is_unlocked_sr_cr(nor, ofs + len, mtd->size - (ofs + len),
					status_old, config_old))
			can_be_bottom = false;
	}
#endif
	...
	printf("sunxi_lock:%d val:0x%x, flag:0x%x\n", lock, val, protect_flag);
	...
	if (lock) {
		/* Only modify protection if it will not unlock other areas */
		if ((status_new & mask) < (status_old & mask)) {
			printf("sunxi lock reject status:0x%x --> 0x%x\n", status_old, status_new);
			return -EINVAL;
		}
	} else {
		/* Only modify protection if it will not lock other areas */
		if ((status_new & mask) > (status_old & mask)) {
			printf("sunxi unlock reject status:0x%x --> 0x%x\n", status_old, status_new);
			return -EINVAL;
		}
	}
#endif
	...
	if (JEDEC_MFR(nor->info) == SNOR_MFR_MACRONIX) {
		...
		printf("sr:0x%x --> 0x%x cr:0x%x --> 0x%x\n",
		...
		printf("sr:0x%x --> 0x%x otp_sr:0x%x --> 0x%x\n",
				status_old, status_new, otp_status_old, otp_status_new);
	} else if (JEDEC_MFR(nor->info) == SNOR_MFR_GIGADEVICE) {
...
{
	return sunxi_handle_lock(nor, ofs, len, 1);

}
static int sunxi_unlock(struct spi_nor *nor, loff_t ofs, uint64_t len)
{
	return sunxi_handle_lock(nor, ofs, len, 0);
}
...
/*
 * S19 bit (RCV) in the Status Register-3 can help to recovery for erase while power drop
 * <AN0000020 Design_Technique_for_Improving_System_Stability_V1.1_05292018.pdf>
 */
static int winbond_config_wps_rcv(struct spi_nor *nor, int wps, int rcv)
{
	int ret = 0, sr3_old, sr3_new;
	sr3_old = read_sr3(nor);
	if (sr3_old < 0)
		return sr3_old;

	sr3_new = sr3_old;
	sr3_new = wps? (sr3_new | SR3_WPS_WB): (sr3_new & ~SR3_WPS_WB);
	sr3_new = rcv? (sr3_new | SR3_RCV_WB): (sr3_new & ~SR3_RCV_WB);

	if (sr3_old != sr3_new) {
		write_enable(nor);
		ret = write_sr3(nor, sr3_new & 0xff);
		spi_nor_wait_till_ready(nor);
	}
	printf("sr3:0x%x --> 0x%x\n", sr3_old, sr3_new);

	return ret;
}

static int winbond_config_individual_mode(struct spi_nor *nor, int mode)
{
	int wps = mode;
	int rcv = 1; /* always enable rcv */
	return winbond_config_wps_rcv(nor, wps, rcv);
}

#endif

#if defined(CONFIG_SPI_FLASH_XT)
static int xtx_config_individual_mode(struct spi_nor *nor, int mode)
{
	int wps = mode;
	...
	ret = read_sr1(nor);
	if (ret < 0) {
		dev_dbg(dev, "error while reading status register-1\n");
		return -EINVAL;
	}
	sr1_old = ret;
	if (wps)
		sr[1] = sr1_old | SR1_WPS_XTX;
	else
		sr[1] = sr1_old & ~SR1_WPS_XTX;
	...
	ret = write_sr_sr1(nor, sr);
	if (ret)
		return ret;

	/* Read back and check it. */
	ret = read_sr1(nor);
	if (!(ret > 0 && ((ret & SR1_WPS_XTX) == (sr[1] & SR1_WPS_XTX)))) {
		dev_dbg(nor->dev, "XTX config WPS bit fail\n");
		return -EINVAL;
	}
...
#if defined(CONFIG_SPI_FLASH_FM)
static int fm_config_individual_mode(struct spi_nor *nor, int mode)
{
	int wps = mode;
	...
	ret = read_sr2(nor);
	if (ret < 0) {
		dev_dbg(dev, "error while reading status register-2\n");
		return -EINVAL;
	}
	sr2_old = ret;
	if (wps)
		sr2 = sr2_old | SR2_WPS_FM;
	else
		sr2 = sr2_old & ~SR2_WPS_FM;
	...
	/* Read back and check it. */
	ret = read_sr2(nor);
	if (!(ret > 0 && ((ret & SR2_WPS_FM) == (sr2 & SR2_WPS_FM)))) {
		dev_dbg(nor->dev, "FM config WPS bit fail\n");
		return -EINVAL;
	}
...
#if defined(CONFIG_SPI_FLASH_XMC)
static int xmc_config_individual_mode(struct spi_nor *nor, int mode)
{
	//TODO
	return 0;
}
...
#if defined(CONFIG_SUNXI_INDIVIDUAL_LOCK)
...
static int sunxi_individual_lock_all(struct spi_nor *nor)
{
	if (JEDEC_MFR(nor->info) == SNOR_MFR_WINBOND) {
		printf("winbond lock all\n");
	} else if (JEDEC_MFR(nor->info) == SNOR_MFR_MACRONIX) {
		printf("mxic lock all\n");
	} else if (JEDEC_MFR(nor->info) == SNOR_MFR_XTX) {
		printf("xtx lock all\n");
	} else if (JEDEC_MFR(nor->info) == SNOR_MFR_FM) {
		printf("fm lock all\n");
	} else if (JEDEC_MFR(nor->info) == SNOR_MFR_XMC) {
		printf("xmc lock all\n");
	} else {
		printf("not support nor 0x%x\n", JEDEC_MFR(nor->info));
		return 0;
	}
	write_enable(nor);
...
static int sunxi_individual_unlock_all(struct spi_nor *nor)
{
	if (JEDEC_MFR(nor->info) == SNOR_MFR_WINBOND) {
		printf("winbond unlock all\n");
	} else if (JEDEC_MFR(nor->info) == SNOR_MFR_MACRONIX) {
		printf("mxic unlock all\n");
	} else if (JEDEC_MFR(nor->info) == SNOR_MFR_XTX) {
		printf("xtx unlock all\n");
	} else if (JEDEC_MFR(nor->info) == SNOR_MFR_FM) {
		printf("fm unlock all\n");
	} else if (JEDEC_MFR(nor->info) == SNOR_MFR_XMC) {
		printf("xmc unlock all\n");
	} else {
		printf("not support nor 0x%x\n", JEDEC_MFR(nor->info));
		return 0;
	}
...
static int sunxi_individual_lock_addr(struct spi_nor *nor, loff_t ofs)
{
	u8 buf[5];
	printf("lock 0x%llx\n", ofs);
	write_enable(nor);
	if (JEDEC_MFR(nor->info) == SNOR_MFR_WINBOND ||
			JEDEC_MFR(nor->info) == SNOR_MFR_XTX ||
			JEDEC_MFR(nor->info) == SNOR_MFR_FM) {
		buf[0] = ofs >> 16;
		buf[1] = ofs >> 8;
		buf[2] = ofs;
		return nor->write_reg(nor, SPINOR_OP_IBLK, buf, 3);
	} else if (JEDEC_MFR(nor->info) == SNOR_MFR_MACRONIX) {
		buf[0] = ofs >> 24;
		buf[1] = ofs >> 16;
		buf[2] = ofs >> 8;
		buf[3] = ofs;
		buf[4] = 0;
		return nor->write_reg(nor, SPINOR_OP_WRDPB, buf, 5);
	} else if (JEDEC_MFR(nor->info) == SNOR_MFR_XMC) {
		//TODO
		printf("not support nor 0x%x\n", JEDEC_MFR(nor->info));
		return 0;
...
static int sunxi_individual_unlock_addr(struct spi_nor *nor, loff_t ofs)
{
	u8 buf[5];
	printf("unlock 0x%llx\n", ofs);
	write_enable(nor);
	if (JEDEC_MFR(nor->info) == SNOR_MFR_WINBOND
			|| JEDEC_MFR(nor->info) == SNOR_MFR_XTX
			|| JEDEC_MFR(nor->info) == SNOR_MFR_FM) {
		...
		buf[1] = ofs >> 8;
		buf[2] = ofs;
		return nor->write_reg(nor, SPINOR_OP_IBULK, buf, 3);
	} else if (JEDEC_MFR(nor->info) == SNOR_MFR_MACRONIX) {
		buf[0] = ofs >> 24;
		buf[1] = ofs >> 16;
		buf[2] = ofs >> 8;
...
static int sunxi_individual_checklock_addr(struct spi_nor *nor, loff_t ofs)
{
	int ret;
	u8 val;
	...
	if (JEDEC_MFR(nor->info) == SNOR_MFR_WINBOND
			|| JEDEC_MFR(nor->info) == SNOR_MFR_XTX
			|| JEDEC_MFR(nor->info) == SNOR_MFR_FM) {
		ret = spi_nor_read_addr_reg(nor, SPINOR_OP_RDBLK, ofs, &val, 1);
	} else if (JEDEC_MFR(nor->info) == SNOR_MFR_MACRONIX) {
		ret = spi_nor_read_addr_reg(nor, SPINOR_OP_RDDPB, ofs, &val, 1);
	} else if (JEDEC_MFR(nor->info) == SNOR_MFR_XMC) {
		//TODO
		printf("not support nor 0x%x\n", JEDEC_MFR(nor->info));
		return 0;
...
static int sunxi_individual_handle_lock(struct spi_nor *nor, loff_t ofs, uint64_t len, int lock)
{
	int ret;
	...
	if ((ofs_lock < NOR_LOCK_BLOCK_SIZE) ||
			(ofs_lock >= nor->size - NOR_LOCK_BLOCK_SIZE))
		ofs_lock &= ~(NOR_LOCK_SECTOR_SIZE - 1);
	else
		ofs_lock &= ~(NOR_LOCK_BLOCK_SIZE - 1);

	if (lock != CHECKLOCK && ofs == 0 && len == nor->size) {
		if (lock == LOCK)
			return sunxi_individual_lock_all(nor);
		else
			return sunxi_individual_unlock_all(nor);
	} else {
		while (ofs_lock < ofs_lock_end) {
			if (lock == LOCK)
				ret = sunxi_individual_lock_addr(nor, ofs_lock);
			else if (lock == UNLOCK)
				ret = sunxi_individual_unlock_addr(nor, ofs_lock);
			else if (lock == CHECKLOCK)
				ret = sunxi_individual_checklock_addr(nor, ofs_lock);

			/* lock/unlock return fail or checklock return locked */
			if (ret)
				return ret;
...
{
	return sunxi_individual_handle_lock(nor, ofs, len, LOCK);
}

static int sunxi_individual_unlock(struct spi_nor *nor, loff_t ofs, uint64_t len)
{
	return sunxi_individual_handle_lock(nor, ofs, len, UNLOCK);
}

static int sunxi_individual_is_locked(struct spi_nor *nor, loff_t ofs, uint64_t len)
{
	return sunxi_individual_handle_lock(nor, ofs, len, CHECKLOCK);
}
#endif
...
static int spi_nor_init(struct spi_nor *nor)
{
	...
	if (JEDEC_MFR(nor->info) == SNOR_MFR_ATMEL ||
		...
		write_sr(nor, 0);
		spi_nor_wait_till_ready(nor);
		/* gd should clear cmp, as cmp=1 sr=0 is actually lock all */
		if (JEDEC_MFR(nor->info) == SNOR_MFR_GIGADEVICE)
			gigadevice_config_cmp(nor, 0);
	}

	if (JEDEC_MFR(nor->info) == SNOR_MFR_WINBOND &&
	    nor->info->flags & SPI_NOR_HAS_LOCK) {
		if (nor->info->flags & SPI_NOR_INDIVIDUAL_LOCK)
			winbond_config_individual_mode(nor, 1);
		else
			winbond_config_individual_mode(nor, 0);
	} else if (JEDEC_MFR(nor->info) == SNOR_MFR_MACRONIX &&
			nor->info->flags & SPI_NOR_HAS_LOCK) {
		if (nor->info->flags & SPI_NOR_INDIVIDUAL_LOCK)
			macronix_config_individual_mode(nor, 1);
		else
			macronix_config_individual_mode(nor, 0);
#if defined(CONFIG_SPI_FLASH_XT)
	} else if (JEDEC_MFR(nor->info) == SNOR_MFR_XTX &&
			nor->info->flags & SPI_NOR_HAS_LOCK) {
		if (nor->info->flags & SPI_NOR_INDIVIDUAL_LOCK)
			xtx_config_individual_mode(nor, 1);
		else
			xtx_config_individual_mode(nor, 0);
#endif
#if defined(CONFIG_SPI_FLASH_FM)
	} else if (JEDEC_MFR(nor->info) == SNOR_MFR_FM &&
			nor->info->flags & SPI_NOR_HAS_LOCK) {
		if (nor->info->flags & SPI_NOR_INDIVIDUAL_LOCK)
			fm_config_individual_mode(nor, 1);
		else
			fm_config_individual_mode(nor, 0);
#endif
#if defined(CONFIG_SPI_FLASH_XMC)
	} else if (JEDEC_MFR(nor->info) == SNOR_MFR_XMC &&
			nor->info->flags & SPI_NOR_HAS_LOCK) {
		if (nor->info->flags & SPI_NOR_INDIVIDUAL_LOCK)
			xmc_config_individual_mode(nor, 1);
		else
			xmc_config_individual_mode(nor, 0);
#endif
	}
	if (nor->quad_enable) {
...
int spi_nor_deinit(struct spi_nor *nor)
{
	/*
	 * Atmel, SST, Intel/Numonyx, and others serial NOR tend to power up
	 * with the software protection bits set
...
int spi_nor_scan(struct spi_nor *nor)
{
...
#endif
#if defined(CONFIG_SPI_FLASH_MACRONIX) || defined(CONFIG_SPI_FLASH_EON) || defined(CONFIG_SPI_FLASH_WINBOND) \
	|| defined(CONFIG_SPI_FLASH_GIGADEVICE) || defined (CONFIG_SPI_FLASH_XT) ||  defined (CONFIG_SPI_FLASH_XMC) \
	|| defined(CONFIG_SPI_FLASH_FM)
	if (info->flags & SPI_NOR_HAS_LOCK) {
		if (info->flags & SPI_NOR_INDIVIDUAL_LOCK) {
			if (JEDEC_MFR(info) == SNOR_MFR_WINBOND ||
					JEDEC_MFR(info) == SNOR_MFR_XTX ||
					JEDEC_MFR(info) == SNOR_MFR_FM ||
					JEDEC_MFR(info) == SNOR_MFR_XMC ||
					JEDEC_MFR(info) == SNOR_MFR_MACRONIX) {
				nor->flash_lock = sunxi_individual_lock;
				nor->flash_unlock = sunxi_individual_unlock;
				nor->flash_is_locked = sunxi_individual_is_locked;
			}
		} else {
			if (JEDEC_MFR(info) == SNOR_MFR_MACRONIX ||
					JEDEC_MFR(info) == SNOR_MFR_EON ||
	...
	if (info->flags & SPI_NOR_HAS_CR_TB)
	...
	printf("SF: Detected %s with flag 0x%x with page size ", nor->name, nor->info->flags);
