...
#ifdef CONFIG_WAIT_RTC_COUNTER_WHEN_SWITCH_RTC_CLK_SRC
static void switch_rtc_clk_src_to_rccal(void)
{
	uint32_t last_hms, current_hms, reg_addr, reg_data;

	reg_addr = 0x40050000 + 0x80;
	reg_data = readl(reg_addr);
	reg_data &= ~(0x3 << 25);
	reg_data |= (0x1 << 25);

	//check HMS value
	current_hms = readl(0x40051014) & 0x1FFFFFFF;
	last_hms = current_hms;

	while (1)
	{
		current_hms = readl(0x40051014) & 0x1FFFFFFF;
		if (current_hms == last_hms)
		{
			continue;
		}
		else
		{
			if ((current_hms - last_hms) >= 1)
			{
				writel(reg_data, reg_addr);
				break;
			}
...
#define DSP_LDO_WORK_VOLT (1225) //400M@1.225V, 274M@1.1V
