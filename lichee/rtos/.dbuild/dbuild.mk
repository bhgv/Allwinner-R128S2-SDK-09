...
CONFIG_HEADER_PATH ?= $(DBUILD_ROOT)/include/generated/$(RTOS_PROJECT_NAME)
CONFIG_HEADER_NAME ?= autoconf.h
...
kconfig-info:
	@echo ""
	@echo "Please install kconfig-frontends"
	@echo "Source code: https://bitbucket.org/nuttx/tools/downloads/kconfig-frontends-4.11.0.1.tar.bz2 (original)"
	@echo "Ubuntu 22.04      : sudo apt install kconfig-frontends"
	@echo "Build from Source : cd tools"
	@echo "                    chmod 777 install_kconfig_frontends.sh"
	@echo "                    sudo ./install_kconfig_frontends.sh"
	@echo ""
...
sub_dir := arch components drivers kernel projects

KBUILD_OBJECTS := $(addprefix $(BUILD_DIR)/,$(addsuffix /obj-in.o,$(sub_dir)))

TOOLCHAIN_LD_FLAGS := $(CONFIG_TOOLCHAIN_LD_FLAGS)

export BASE
export BUILD_DIR
export TOOLCHAIN_LD_FLAGS
export DBUILD_OS
...
EXPORT_ASFLAGS += $(ASFLAGS)

.PHONY: $(KBUILD_OBJECTS) FORCE
$(KBUILD_OBJECTS): $(CONFIG_HEADER_PATH)/$(CONFIG_HEADER_NAME) FORCE
	$(Q)set -e
	$(Q)$(MAKE) -f $(BASE)/scripts/build/Makefile.build \
		EXPORT_LDFLAGS="$(EXPORT_LDFLAGS)" EXPORT_CFLAGS="$(EXPORT_CFLAGS)" \
		EXPORT_CXXFLAGS="$(EXPORT_CXXFLAGS)" EXPORT_ASFLAGS="$(EXPORT_ASFLAGS)" \
		dir=$(notdir $(patsubst %/,%,$(dir $@))) out=$(BUILD_DIR)/
...
ifeq ($(DBUILD_OS), WIN32)
MCONF:=cmd /c start /wait scripts/kconfig-win32/kconfig-mconf.exe
endif
...
ifneq ($(RTOS_TARGET_PROJECT_PATH),)
	@cp ${BASE}/projects/${RTOS_TARGET_PROJECT_PATH}/defconfig ${BASE}/.config
	cd $(BASE)/ && $(MCONF_TERM) CONFIG_=$(CONFIG_) PROJECT_DIR=$(PROJECT_DIR) $(MCONF) Kconfig
	@cp ${BASE}/.config ${BASE}/projects/${RTOS_TARGET_PROJECT_PATH}/defconfig
else
	cd $(BASE)/ && $(MCONF_TERM) CONFIG_=$(CONFIG_) PROJECT_DIR=$(PROJECT_DIR) $(MCONF) Kconfig
endif
	$(MAKE) mkconfig
...
.PHONY: ota_menuconfig
ota_menuconfig: $(DBUILD_ROOT).dbuild/scripts/mkconfig/mkconfig$(OS_EXT)
ifneq ($(DBUILD_OS), WIN32)
ifneq ($(DBUILD_OS), LINUX_64)
	@which kconfig-mconf > /dev/null || { echo "*** kconfig-frontends is not installed"; $(MAKE) kconfig-info ; false; }
endif
...
ifneq ($(RTOS_TARGET_PROJECT_PATH),)
	@cp ${BASE}/projects/${RTOS_TARGET_PROJECT_PATH}/ota_defconfig ${BASE}/.config
	cd $(BASE)/ && $(MCONF_TERM) CONFIG_=$(CONFIG_) PROJECT_DIR=$(PROJECT_DIR) $(MCONF) Kconfig
	@cp ${BASE}/.config ${BASE}/projects/${RTOS_TARGET_PROJECT_PATH}/ota_defconfig
else
	cd $(BASE)/ && $(MCONF_TERM) CONFIG_=$(CONFIG_) PROJECT_DIR=$(PROJECT_DIR) $(MCONF) Kconfig
endif
	$(MAKE) mkconfig
