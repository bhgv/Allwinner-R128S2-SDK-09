    ...
    if( pxTaskStatusArray != NULL )
    {
        ...
        for( i = 0; i < configNR_CPUS; i++ )
        {
            ...
            for( x = 0; x < uxArraySize; x++ )
            {
                ...
                stk_usage = (float)(temp->pxStackSize - ((unsigned long) ptr - (unsigned long) temp->pxStackBase)) * 100 / (float)temp->pxStackSize;
#else
                uint8_t * ptr = (uint8_t *)temp->pxEndOfStack;
                stk_usage = (float)(temp->pxStackSize - ((unsigned long) temp->pxStackBase - (unsigned long)ptr)) * 100 / (float)temp->pxStackSize;
#endif
