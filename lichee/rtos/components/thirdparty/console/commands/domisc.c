...
#ifdef CONFIG_COMPONENTS_ENV_SETENV
		} else if (!strcmp(argv[1], "normal")) {
			printf("**** Jump to normal! *****\n");
			int cmd_fw_setenv(int argc, char **argv);
			char *env_argv[] = {
				"fw_setenv",
				"bootcmd",
				"run boot_normal"};
			int ret = cmd_fw_setenv(3, env_argv);
			printf("setenv return %d\n", ret);
		} else if (!strcmp(argv[1], "etf")) {
			printf("**** Jump to etf! *****\n");
			int cmd_fw_setenv(int argc, char **argv);
			char *env_argv[] = {
				"fw_setenv",
...
#define CCMU_AON_BASE 0x4004c400
    ...
    /* force wlan soc reset and disable wlan cpu clk */
    sr32(CCMU_AON_BASE + 0xc8, 12, 1, 0x0);
    HAL_PRCM_EnableWlanCPUClkOvrHIF();
    HAL_PRCM_EnableResetWlanCPUOvrHIF();
    HAL_PRCM_ForceWlanCPUReset();
    HAL_PRCM_EnableWlanCPUClk(0); /* '0' is disable */
#endif
#undef CCMU_AON_BASE
#endif
...
int cmd_free(int argc, char ** argv)
{
    ...
    for (i = 0; i < num; i ++) {
        printf("==> Round [%d] <==\n", i+1);
#if defined(CONFIG_HEAP_MULTIPLE)
...
#endif
#elif defined(CONFIG_HEAP_MULTIPLE_DYN)
        extern int aw_get_default_heapID(void);
        int default_heapID = aw_get_default_heapID();
        show_heap_info(default_heapID, 0, "default");
#ifdef CONFIG_SRAM
        show_heap_info(SRAM_HEAP_ID, 0, "sram");
#endif
#ifdef CONFIG_DRAM
        show_heap_info(DRAM_HEAP_ID, 0, "dram");
#endif
#ifdef CONFIG_LPSRAM
        show_heap_info(LPSRAM_HEAP_ID, 0, "lpsram");
#endif
#ifdef CONFIG_HPSRAM
        show_heap_info(HPSRAM_HEAP_ID, 0, "hpsram");
#endif
#else
        show_heap_info();
#endif
        char pcHeader[2048] = "Task          State  Priority  Stack      #\r\n************************************************\r\n";
        printf("\n      List Task MIN Free Stack( unit: %zu Bytes )      \n", sizeof(BaseType_t));
