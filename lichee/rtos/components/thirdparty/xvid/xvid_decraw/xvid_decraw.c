...
int cmd_xvid_decraw_main(int argc, char *argv[])
{
	...
	do {
		...
		do {
			...
			if(used_bytes > 0) {
				...
				printf("[%s %d]Data chunk %d: %d bytes consumed, %d bytes in buffer\n", __func__, __LINE__, chunk++, used_bytes, useful_bytes);
		...
		if (ARG_SAVEDECOUTPUT) {
			sprintf(filename, "/data/%sdec", filepath);
            if (save_cnt_tmp == 0) {
            printf("[THH] going to save one frame\n");
...
FINSH_FUNCTION_EXPORT_CMD(cmd_xvid_decraw_main, xvid, testxvid);
