...
#define FIRST_PKT  1
#define HCI_TX_TIMEOUT_MS 10000

/* define traces for vhci */
#define VHCI_TRACE_LEVEL_ERROR   (0)
#define VHCI_TRACE_LEVEL_WARNING (1)
#define VHCI_TRACE_LEVEL_DEBUG   (2)

#define VHCI_INITIAL_TRACE_LEVEL VHCI_TRACE_LEVEL_ERROR

#define VHCI_TRACE_ERROR(fmt, args...)       {if (VHCI_INITIAL_TRACE_LEVEL >= VHCI_TRACE_LEVEL_ERROR)   printf("<VHCI_E>" fmt, ##args);}
#define VHCI_TRACE_WARNING(fmt, args...)     {if (VHCI_INITIAL_TRACE_LEVEL >= VHCI_TRACE_LEVEL_WARNING) printf("<VHCI_W>" fmt, ##args);}
#define VHCI_TRACE_DEBUG(fmt, args...)       {if (VHCI_INITIAL_TRACE_LEVEL >= VHCI_TRACE_LEVEL_DEBUG)   printf("<VHCI_D>" fmt, ##args);}
               ...
               unsigned int offset, unsigned int len)
{
    ...
    if ((hci_type != HCI_H4_ACL) && (hci_type != HCI_H4_EVT) && (hci_type != HCI_H4_SCO)) {
        VHCI_TRACE_ERROR("hci c2h hci type errr: %d\n", hci_type);
        goto fail;
    }

    hci_pkt = (hci_pkt_t *)malloc(sizeof(hci_pkt_t));
    if (!hci_pkt) {
        VHCI_TRACE_ERROR("vhci_c2h hci_pkt malloc fail\n");
        goto fail;
    }
    hci_pkt->buff = (uint8_t *)malloc(len + 1);
    if (!hci_pkt->buff) {
        VHCI_TRACE_ERROR("vhci_c2h hci_pkt->buff malloc fail, %d\n", len + 1);
        free(hci_pkt);
        goto fail;
    }
...
static int drv_init()
{
    ...
    if (ret) {
        VHCI_TRACE_ERROR("tx sem create failed\n");
        return VHCI_FAILED;
    }
    ...
    if (ret) {
        VHCI_TRACE_ERROR("rx sem create failed\n");
        goto fail_rx_sem;
    }

    ret = XR_OS_MutexCreate(&rx_mutex);
    if (ret) {
        VHCI_TRACE_ERROR("rx mutex create failed\n");
        goto fail_mutex;
    }
    ...
    if (ret = XR_OS_ThreadCreate(&rx_thread, "rx_thread", rx_thread_handle, NULL, HCI_RX_THREAD_PROI, HCI_RX_THREAD_STACK_SIZE) != XR_OS_OK) {
        VHCI_TRACE_ERROR("uart_rx_thread create failed\n");
        goto fail_thread;
    }

    ptr_pkt = hci_list_new(HCI_PKT_TYPE);
    if(NULL == ptr_pkt) {
        VHCI_TRACE_ERROR("list create failed\n");
        goto fail_list;
    }
...
static int drv_deinit()
{
    ...
    if (ret) {
        VHCI_TRACE_ERROR("tx sem delete failed\n");
    }

    ret = XR_OS_SemaphoreDelete(&rx_sem);
    if (ret) {
        VHCI_TRACE_ERROR("rx sem delete failed\n");
    }

    ret = XR_OS_ThreadDelete(&rx_thread);
    if (ret) {
        VHCI_TRACE_ERROR("thread delete failed\n");
    }

    ret = XR_OS_MutexDelete(&rx_mutex);
    if (ret) {
        VHCI_TRACE_ERROR("rx mutex delete failed\n");
    }
...
{
    int ret;
    if (XR_OS_SemaphoreWait(&tx_sem, HCI_TX_TIMEOUT_MS) == XR_OS_OK) {
        ret = xrbtc_hci_h2c(data[0], data + 1, 0, len - 1);
        if (ret != 0) {
            VHCI_TRACE_ERROR("h2c process fail\n");
            XR_OS_SemaphoreRelease(&tx_sem);
        }
    } else {
        VHCI_TRACE_ERROR("h2c cb timeout\n");
        ret = VHCI_FAILED;
    }
