...
int hal_workqueue_destroy(hal_workqueue *queue)
{
    ...
    unsigned long flag;
    TaskStatus_t work_thread_status;

    vTaskGetInfo(queue->work_thread, &work_thread_status, pdFALSE, eInvalid);

    flag = workqueue_list_lock(queue);
    if ((queue->work_current == NULL) && (work_thread_status.eCurrentState == eSuspended)) {
        workqueue_list_unlock(queue, flag);
