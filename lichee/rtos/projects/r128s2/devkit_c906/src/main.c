...
#ifdef CONFIG_COMPONENTS_AW_ANTI_COPY_BOARD
...
static void print_banner()
{
    ...
    printf(" **                                       **\r\n");
    printf(" **      starting riscv FreeRTOS V0.9     **\r\n");
...
#ifdef CONFIG_COMPONENTS_AW_ANTI_COPY_BOARD
	int ret1 = -1;
	ret1 = acb_verify(PLAINTEXT_PATH, CIPHERTEXT_PATH);
	if(ret1) {
		if (ret1 == ACB_VERIFY_RET_NO_ALL_ERR) {
			printf("not verity file and not burn ssk, jump verity\n");
		} else {
			printf("acb verity fail!\n");
			cmd_reboot(0, NULL);
		}
    } else {
