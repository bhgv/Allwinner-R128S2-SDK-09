...
#define _SUNXI_CCU_FRAC_MULT(_shift, _width) \
    {                               \
                                      .shift  = _shift,                   \
                                                .width  = _width,                   \
    }
...
struct ccu_frac_mult
{
    ...
    struct ccu_frac_internal    frac;
    struct ccu_mult_internal    mult;
    struct ccu_pllfrac_internal pllfrac;
    struct ccu_mux_internal mux;
    struct ccu_common   common;
};
...
#define SUNXI_CCU_N_WITH_GATE_LOCK_OFFSET_REVERSE(_struct, _name, _parent, _reg,   \
                                   _mshift, _mwidth, _offset, _fshift, _fwidth,	\
				  _gate, _lock,  \
                                   _flags)              \
struct ccu_frac_mult _struct = {                 \
    .enable = _gate,                    \
              .lock   = _lock,                    \
                        .mult   = _SUNXI_CCU_MULT_OFFSET(_mshift, _mwidth, _offset),        \
                        .pllfrac   =  _SUNXI_CCU_FRAC_MULT(_fshift, _fwidth),        \
                                  .common = {                     \
								  .features    = CCU_FEATURE_DPLL_FRAC_REVERSE, \
                                                                  .reg        = _reg,             \
                                                                  .hw.init    = CLK_HW_INIT(_name,        \
                                                                          _parent,      \
                                                                          &ccu_frac_mult_ops,    \
...
static inline struct ccu_frac_mult *hw_to_ccu_frac_mult(struct clk_hw *hw)
{
    struct ccu_common *common = hw_to_ccu_common(hw);

    return container_of(common, struct ccu_frac_mult, common);
}

extern const struct clk_ops ccu_mult_ops;
extern const struct clk_ops ccu_frac_mult_ops;
