...
static unsigned int ccu_dpllfrac_bit_reverse(unsigned int dpll_frac, struct ccu_frac_mult *frac_mult)
{
	int i;
	unsigned int real_value = 0;

	for (i = 0; i < frac_mult->pllfrac.width; i++) {
		if ((dpll_frac & (1 << i)) != 0)
			real_value |= (1 << (frac_mult->pllfrac.width - 1 - i));
	}
...
static void ccu_mult_pllfrac_find_best(unsigned long parent, unsigned long rate,
		struct _ccu_mult *mult, struct ccu_frac_mult *frac_mult, struct clk_hw *hw)
{
	int _mult;
	u32 reg;
	unsigned long long pllfrac;
	unsigned int real_pllfrac;
	...
	if (pllfrac != 0) {
		...
		real_pllfrac = (pllfrac * 65536) / 1000000;

		/* write DPLL_FRAC bit[13:28] */
		if (frac_mult->common.features & CCU_FEATURE_DPLL_FRAC_REVERSE) {
			real_pllfrac = ccu_dpllfrac_bit_reverse(real_pllfrac, frac_mult);
			set_reg(frac_mult->common.base + frac_mult->common.reg, real_pllfrac, frac_mult->pllfrac.width, frac_mult->pllfrac.shift);
		} else {
			set_reg(frac_mult->common.base + frac_mult->common.reg, real_pllfrac, frac_mult->pllfrac.width, frac_mult->pllfrac.shift);
		}
		/* write DPLL_FRAC_CTRL bit12 */
		set_reg(frac_mult->common.base + frac_mult->common.reg, 0x1, 1, 12);
	}
		...
		unsigned long parent_rate)
{
	...
	ccu_mult_pllfrac_find_best(parent_rate, rate, &_cm, cm, hw);
