...
#define LCD_SPI_MAX_TRANSFER_BYTE (4096)
...
static s32 lcd_parse_panel_para(u32 disp, struct disp_panel_para *info)
{
	...
	if (info->lcd_if == LCD_FB_IF_DBI) {
#if !defined(SUPPORT_DBI_IF)
		lcd_fb_wrn("NOT SUPPORT DBI! Please check it in file:lcd_fb_feature.h\n");
		info->lcd_if = LCD_FB_IF_SPI;
#endif
	}
	...
	info->line_length = info->lcd_x * Bpp;

	if (info->lcd_if == LCD_FB_IF_SPI) {
		info->lines_per_transfer = 1;
		lines = LCD_SPI_MAX_TRANSFER_BYTE / (info->line_length);
		for (; lines > 1; --lines) {
			if (!(info->lcd_y % lines))
				break;
		}
...
static s32 disp_lcd_backlight_disable(struct lcd_fb_device *lcd)
{
	...
	if (disp_lcd_is_used(lcd)) {
		if (lcdp->lcd_cfg.lcd_bl_en_used) {
			if (lcdp->lcd_cfg.lcd_bl_gpio_hdl)
...
#else
		lcd_fb_wrn("Please Enable Driver Config: CONFIG_DRIVERS_DBI !!\n");
		return -1;
#endif
	} else {
#if defined(CONFIG_DRIVERS_SPI)
...
#else
		lcd_fb_wrn("Please Enable Driver Config: CONFIG_DRIVERS_SPI !!\n");
		return -1;
#endif
	}
		...
		void *buf, uint32_t size)
{
	...
	hal_set_dbi_config(port, &dbim.cfg);
...
static int disp_lcd_dbi_pixel_cfg(struct lcd_fb_device *lcd)
{
	dbi_master_status_t ret;
	struct disp_lcd_private_data *lcdp = disp_lcd_get_priv(lcd);
	if (!lcd || !lcdp) {
	...
	ret = hal_set_dbi_config(dbim.port, &dbim.cfg);
	return ret;
}

static int disp_lcd_send_pixel_by_dbi(hal_dbi_master_port_t port,
...
#if defined(CONFIG_DRIVERS_SPI)
static int disp_lcd_read_by_spi(int port,
...
static int disp_lcd_write_by_spi(int port,
		const void *buf, uint32_t size)
{
	hal_spi_master_status_t ret;
	hal_spi_master_transfer_t t;
...
static int disp_lcd_send_pixel_by_spi(int port,
...
#if defined(CONFIG_DRIVERS_DBI)
		ret = disp_lcd_send_pixel_by_dbi(dbim.port, buf, len);
		if (ret)
...
int disp_lcd_set_layer(struct lcd_fb_device *lcd, struct fb_info *p_info)
{
	...
	if (!p_info->fix.line_length) {
		p_info->fix.line_length = lcdp->panel_info.line_length;
	}
	...
	if (lcdp->panel_info.lcd_if == LCD_FB_IF_DBI) {
		...
		p_info->fix.smem_len = p_info->fix.line_length * p_info->var.yres;
		len = p_info->fix.line_length;
...
void lcd_fb_black_screen(unsigned int sel)
{
	...
	screensize = info[sel].line_length * info[sel].lcd_y;
