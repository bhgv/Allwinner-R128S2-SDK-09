...
#ifdef CONFIG_COMPONENTS_PM
static struct pm_wakesrc *rtc_ws = NULL;
#endif
...
#ifdef CONFIG_COMPONENTS_PM
    if (!pm_wakesrc_is_inexistent("rtc_alarm")) {
        rtc_ws = pm_wakesrc_register(rtc_dev->irq, "rtc_alarm", PM_WAKESRC_ALWAYS_WAKEUP);
   	if (!rtc_ws) {
	   RTC_ERR("rtc registers wakesrc fail\n");
	} else {
	   ret = pm_set_wakeirq(rtc_ws);
	   if (ret)
	       RTC_ERR("rtc pm_set_wakeirq fail\n");
        }
    } else
    	rtc_ws = NULL;
#endif
...
#ifdef CONFIG_COMPONENTS_PM
    if (rtc_ws) {
    	if (pm_clear_wakeirq(rtc_ws))
	    RTC_ERR("rtc pm_clear_wakeirq fail\n");
    	if (pm_wakesrc_unregister(rtc_ws))
	    RTC_ERR("rtc unregisters wakesrc fail\n");
    }
#endif
