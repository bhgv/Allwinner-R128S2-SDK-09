...
int nor_transfer(int single_len, void *tbuf, int tlen, void *rbuf, int rlen)
{
    ...
    if (unlikely(hal_interrupt_get_nest() != 0))
        ret = hal_spi_panic_xfer(nor->spim.port, &tr, 1);
    else
#endif
        ret = hal_spi_xfer(nor->spim.port, &tr, 1);
    if (ret)
...
#ifdef CONFIG_DRIVERS_SPINOR_PANIC_WRITE
    if (unlikely(hal_interrupt_get_nest() != 0))
        return nor_wait_ready(0, MAX_WAIT_LOOP);
    else
#endif
...
#ifdef CONFIG_DRIVERS_SPINOR_PANIC_WRITE
int nor_panic_write(unsigned int addr, char *buf, unsigned int len)
{
    int ret;
    ...
    if (nor_wait_ready(0, MAX_WAIT_LOOP))
        goto out;
...
int nor_panic_read(unsigned int addr, char *buf, unsigned int len)
{
    int ret;
...
static inline int nor_panic_erase_4k(unsigned int addr)
{
    int ret;
    ...
    ret = nor_wait_ready(0, MAX_WAIT_LOOP);
out:
#ifdef CONFIG_DRIVERS_SPINOR_WRITE_LOCK
    nor_wr_lock_all(nor);
#endif
...
static inline int nor_panic_erase_32k(unsigned int addr)
{
    int ret;
...
static inline int nor_panic_erase_64k(unsigned int addr)
{
    int ret;
...
static inline int nor_panic_erase_all(void)
{
    int ret;
