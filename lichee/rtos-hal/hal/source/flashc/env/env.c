 ...
 */
#ifdef SUPPORT_FLASHC_WRITE_ENV
int flashc_env_write(char *name, char *value)
{
	int len;
	char *env, *nxt;
	char *oldval = NULL;
...
{
#if 1
	int ret, start_sector, sectors;

	if (dev_current == 0) {
		ret = get_part_info_by_name(env_gpt_buff, "env", &start_sector, &sectors);
		if (ret) {
			return -1;
		}
	} else if (dev_current == 1) {
		ret = get_part_info_by_name(env_gpt_buff, "env-redund", &start_sector, &sectors);
		if (ret) {
			return -1;
		}
	} else {
	...
	ret = nor_read(start_sector * 512, buf, size);
	if (ret) {
	...
	if (blkpart_read(env, 0, size, buf) != size) {
		printf("read env fail\n");
		return -1;
	}
#endif
...
static int flash_env_write(void *buf, size_t size)
{
	...
	int fd, len;
	if (dev_current == 1) {
		fd = open("/dev/env", O_RDWR);
		if (fd == -1) {
			printf("open part env fail\n");
			return -1;
		}
	} else if (dev_current == 0) {
		fd = open("/dev/env-redund", O_RDWR);
		if (fd == -1) {
			printf("open part env-redund fail\n");
			return -1;
		}
	} else {
		printf("invalid dev_current:%d\n", dev_current);
	...
	//printf("dev_current:%d, write len:%d, size:%d\n", dev_current, len, size);
	fsync(fd);
	close(fd);
	if (len != size) {
	...
	if (blkpart_erase(env_write, 0, size)) {
		printf("erase env fail\n");
		return -1;
	}

	if (blkpart_write(env_write, 0, size, buf) != size) {
		printf("write env fail\n");
		return -1;
	}

	if (blkpart_sync(env_write)) {
                printf("sync env fail\n");
                return -1;
        }
		...
		if (crc0_ok && !crc1_ok) {
			...
			switch (environment.flag_scheme) {
				...
				case FLAG_INCREMENTAL:
					...
					break;
				default:
					fprintf(stderr, "Unknown flag scheme %d\n",
						environment.flag_scheme);
					return -1;
			}
		}
...
int flashc_get_xip_part_name_from_env(char *gpt_buf, char *buff)
{
	int ret;
	char *tmp = NULL;

	env_gpt_buff = gpt_buf;
	if (buff == NULL)
		return -1;

	ret = flashc_env_open();
	if (ret) {
		printf("flashc open env failed!\n");
		return -1;
	}

	tmp = flashc_getenv("xip_partition");
	if (tmp == NULL) {
		printf("get xip partition name from env failed!\n");
		goto exit;
	}

	memset(buff, 0, PART_NAME_MAX_LEN);
	memcpy(buff, tmp, strlen(tmp));
	printf("xip partition name is :%s\n", buff);
