...
static const uint32_t hal_twi_address[] =
{
    ...
    SUNXI_TWI2_PBASE,
    SUNXI_TWI3_PBASE,
#if (defined(CONFIG_ARCH_SUN20IW3) || defined(CONFIG_ARCH_SUN55IW3))
    SUNXI_TWI4_PBASE,
#endif
#ifdef CONFIG_ARCH_SUN55IW3
    SUNXI_TWI5_PBASE,
#endif
#if !(defined(CONFIG_ARCH_SUN8IW20) || defined(CONFIG_SOC_SUN20IW1) || defined(CONFIG_ARCH_SUN20IW2) || defined(CONFIG_ARCH_SUN20IW3))
    SUNXI_S_TWI0_PBASE,
#endif
#ifdef CONFIG_ARCH_SUN55IW3
    SUNXI_S_TWI1_PBASE,
...
#if !(defined(CONFIG_ARCH_SUN8IW20) || defined(CONFIG_SOC_SUN20IW1) || defined(CONFIG_ARCH_SUN20IW2) || defined(CONFIG_ARCH_SUN20IW3) || defined(CONFIG_ARCH_SUN55IW3))
...
err_out:
    twi->msgs_idx = code;
    twi->result = RESULT_ERR;
    TWI_ERR("packet transmission failed , status : 0x%0u", code);
...
# ifdef CONFIG_ARCH_SUN55IW3
	...
	if(hal_efuse_get_chip_ver() == CHIP_VER_C)
		gpio_level = TWI_DRIVE_STATE;
	else
		gpio_level = 1;
#else
        gpio_level = TWI_DRIVE_STATE;
#endif
        if (hal_gpio_set_driving_level(twi->pin[i], gpio_level))
        {
...
twi_status_t hal_twi_uninit(twi_port_t port)
{
    ...
    if (twi->already_init > 0)
    {
        ...
        if (twi->already_init == 0)
        {
            ...
            hal_twi_clk_exit(twi);
#if !(defined(CONFIG_ARCH_SUN8IW18P1) || defined(CONFIG_SOC_SUN20IW1) || defined(CONFIG_ARCH_DSP))
            hal_twi_regulator_exit(twi);
#endif
            hal_sem_delete(twi->hal_sem);
        }
...
twi_status_t hal_twi_write(twi_port_t port, twi_msg_t *msg, uint32_t msg_num)
{
    ...
    status = hal_twi_xfer(port, msg, msg_num);
...
twi_status_t hal_twi_read(twi_port_t port, hal_twi_transfer_cmd_t cmd, uint16_t slave_addr, uint8_t reg_addr, void *buf, uint32_t size)
{
    ...
    status = hal_twi_xfer(port, msg, num);
unlock:
    hal_mutex_unlock(twi->lock);
