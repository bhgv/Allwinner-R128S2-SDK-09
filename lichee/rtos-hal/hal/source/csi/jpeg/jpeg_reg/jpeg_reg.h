...
struct  jpeg_reg {
	...
	__IO uint32_t CSI_OUTPUT_MBUS_EN;	/*!< 0x2C JPEG CSI output bus1 enable register */
	__IO uint32_t CSI_OUTPUT_MBUS_START_ADDR;	/*!< 0x30 JPEG CSI output bus1 start address register */
	__IO uint32_t CSI_OUTPUT_MBUS_END_ADDR;	/*!< 0x34 JPEG CSI output bus1 end address register */

		 uint32_t RESERVED1[242];
