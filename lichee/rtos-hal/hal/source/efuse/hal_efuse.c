...
int hal_efuse_write_ext(uint32_t start_bit, uint32_t bit_num, uint8_t *data)
{
	if ((data == NULL) ||
		...
		(bit_num > EFUSE_BIT_NUM) ||
		(start_bit + bit_num > EFUSE_BIT_NUM)) {
		EFUSE_ERR("arg error: start bit %u, bit num %u, data 0x%p\n", (unsigned int)start_bit, (unsigned int)bit_num, data);
		return EFUSE_ERR_ARG;
	}

	uint8_t *p_data = data;
	uint32_t word_idx = start_bit / 32;
	uint8_t bit_shift = start_bit % 32;
	uint64_t buf = 0;
	...
	if(efuse_uni_burn_key((uint8_t)word_idx * 4, efuse_word[1]))
		return -1;
...
void hal_efuse_set_rd_fbd_bit(int bit_offset)
{
	efuse_set_cfg_flg(EFUSE_READ_PROTECT, bit_offset);
}
