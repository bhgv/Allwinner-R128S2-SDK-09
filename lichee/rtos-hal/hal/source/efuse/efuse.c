...
//set ext_ldo voltage to 2.7V
	...
	reg_val = hal_readl(SUNXI_PRCM_EXT_LDO_CTRL);
	reg_val &= ~(0x2 << 4);
	reg_val |= 0x20;
	hal_writel(reg_val, SUNXI_PRCM_EXT_LDO_CTRL);
	hal_udelay(200);
#endif
...
uint efuse_reg_read_key(uint key_index)
{
#ifdef CONFIG_ARCH_SUN20IW2
	/* for aw1883, reading another reg before reading target reg
	 * to bypass hw errors. */
	_efuse_reg_read_key(0x84);
#endif
	return _efuse_reg_read_key(key_index);
}
