...
static int ScsiDoCommand(__mscLun_t *mscLun, __ScsiCmnd_t *ScsiCmnd)
{
	...
	if (mscLun == NULL || ScsiCmnd == NULL) {
		hal_log_err("ERR: ScsiDoCommand: input error, mscLun = %p, ScsiCmd == %p",
			    mscLun, ScsiCmnd);
...
int ScsiInquery(__mscLun_t *mscLun, void *buffer, unsigned int buffer_len)
{
	...
	if (mscLun == NULL || buffer == NULL) {
		hal_log_err("ERR: ScsiInquery: input error, mscLun = %p, buffer = %p",
			    mscLun, buffer);
		return USB_STATUS_BAD_ARGUMENTS;
	}
...
int ScsiReadCapacity(__mscLun_t *mscLun)
{
	__ScsiCmnd_t ScsiCmnd;
	unsigned char CB[MAX_CDB];
	unsigned char buffer[8] __attribute__((aligned(64)));;
	int ret = 0;

	if (mscLun == NULL) {
		hal_log_err("ERR: ScsiInquery: input error, mscLun = %p", mscLun);
		return USB_STATUS_BAD_ARGUMENTS;
	}
...
static int ScsiGetSenseData(__mscLun_t *mscLun)
{
	__SenseData_t SenseData   __attribute__((aligned(64)));
	int ret = 0;
	...
	hal_log_info("SenseKey = %x, AdditionalSenseCode = %x",
