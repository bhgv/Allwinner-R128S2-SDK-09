	...
	while (1) {
		...
		for (i = 0; i < mscDev->MaxLun; i++) {
			...
			if (!mscDevOnline(mscDev)) {
				hal_log_info("MediaChangeThread MSC_DEV_OFFLINE");
				goto thread_exit;
			}
		}
...
thread_exit:
	// hal_log_info("MediaChangeThread exit");
	hal_sem_post(mscDev->notify_complete);
#ifdef CONFIG_KERNEL_FREERTOS  // melis exit auto
	/*safe exit*/
	while (1) {
		if (!hal_thread_stop(mscDev->MediaChangeId))
			break;
		hal_msleep(3);
	}
#endif
...
int mscDevQueueCmnd(__mscLun_t *mscLun, __ScsiCmnd_t *ScsiCmnd)
{
	...
	if (mscLun == NULL) {
		hal_log_err("ERR: MscTransport: input error, mscLun = 0x%p, ScsiCmnd = %p", mscLun, ScsiCmnd);
...
int mscInit(void)
{
	...
	if (!mutex_lock) {
		hal_log_err("ERR: mscInit: create mutex lock failed");
		return -1;
	}
	return 0;
}
