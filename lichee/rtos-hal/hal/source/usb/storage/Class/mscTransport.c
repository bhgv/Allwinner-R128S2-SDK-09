...
static void mscTimeOut(void *parg)
{
	...
	hal_log_info("ERR: mscTimeOut: CurrentUrb = %p", mscDev->CurrentUrb);
...
static int mscUsbTransport(__mscDev_t *mscDev, unsigned int TimeOut)
{
	...
	if (mscDev == NULL) {
		hal_log_err("ERR: mscUSBTransport: input error, mscDev = %p", mscDev);
		return -EINVAL;
	}
...
{
	if (mscDev == NULL) {
		hal_log_err("ERR: USBStorageCtrlMsg : mscDev = %p", mscDev);
...
unsigned int mscGetMaxLun(__mscDev_t *mscDev)
{
	...
	if (mscDev == NULL) {
		hal_log_err("ERR: GetMaxLun: input error, mscDev = %p", mscDev);
		return 0;
	}
...
static int mscPortReset(__mscDev_t *mscDev)
{
	...
	if (mscDev == NULL) {
		hal_log_err("ERR: mscPortReset: input error, mscDev = %p", mscDev);
		return USB_ERR_BAD_ARGUMENTS;
	}
...
int mscBoTransport(__mscDev_t *mscDev, __ScsiCmnd_t *ScsiCmnd)
{
	...
	if (mscDev == NULL || ScsiCmnd == NULL) {
		hal_log_err("ERR: mscBoTransport: input error, mscDev = %p,ScsiCmnd = %p", mscDev, ScsiCmnd);
...
RETRY_CSW:
	...
	if (sizeof(__CSW_t) != ActLen) {
		hal_log_err("ERR: Invalid Csw size: (%zu, %d), must reset", sizeof(__CSW_t), ActLen);
		mscDev->ResetRecovery(mscDev);
		ret = USB_STOR_TRANSPORT_ERROR;
		goto TransportDone;
	}
...
{
	if (mscDev == NULL) {
		hal_log_err("ERR: mscBoStopTransport: input error, mscDev = %p", mscDev);
		return USB_ERR_BAD_ARGUMENTS;
	}
