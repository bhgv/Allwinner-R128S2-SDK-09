...
__attribute__((__unused__)) static int32_t usbh_disk_info_ioctrl(struct devfs_node *node, int Cmd, void *args[2]) // fix unused-function
{
	usbh_disk_info_t *info = &usbh_disk_info;
	...
	if (node->private != (void *)info) {
		hal_log_err("ERR : handle error, (0x%p)\n", info);
		return EPDK_FAIL;
	}
