...
static void insmod_host_driver(struct usb_msg_center_info *center_info)
{
	...
	printf("[usb0] insmod host driver!\n");
...
void hal_usb_set_otg_role(enum usb_role role)
{
	struct usb_msg_center_info *center_info = &g_center_info;

	set_usb_role(center_info, role);
}
int hal_usb_get_otg_role(void)
{
	return (int)get_usb_role();
}
