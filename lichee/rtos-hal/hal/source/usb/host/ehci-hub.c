...
static void ehci_handover_companion_ports(struct ehci_hcd *ehci)
{
	...
	while (port--) {
		if (usb_test_bit(port, (volatile uint32_t *)&ehci->owned_ports)) {
	...
	while (port--) {
		if (usb_test_bit(port, (volatile uint32_t *)&ehci->owned_ports)) {
			...
			if (status & PORT_OWNER)
				usb_clear_bit(port, (volatile uint32_t *)&ehci->owned_ports);
			else if (usb_test_bit(port, (volatile uint32_t *)&ehci->companion_ports))
...
// 		port = HCS_N_PORTS(ehci->hcs_params);
// 		while (port--) {
// 			u32 *hostpc_reg = &ehci->regs->hostpc[port];
...
// 	port = HCS_N_PORTS(ehci->hcs_params);
// 	while (port--) {
// 		u32 *reg = &ehci->regs->port_status[port];
...
int ehci_bus_suspend(struct usb_hcd *hcd)
{
	...
	while (port--) {
		...
		if (t1 & PORT_OWNER)
			usb_set_bit(port, (volatile uint32_t *)&ehci->owned_ports);
		else if ((t1 & PORT_PE) && !(t1 & PORT_SUSPEND)) {
			t2 |= PORT_SUSPEND;
			usb_set_bit(port, (volatile uint32_t *)&ehci->bus_suspended);
		}
		...
		if (t1 != t2) {
			 ...
			 */
			if ((t2 & PORT_WKDISC_E)
			    && ehci_port_speed(ehci, t2) == USB_PORT_STAT_HIGH_SPEED)
				fs_idle_delay = true;
			ehci_writel(ehci, t2, reg);
			changed = 1;
		}
	...
	if (changed && ehci->has_tdi_phy_lpm) {
		...
		while (port--) {
			...
			ehci_dbg("Port %d phy low-power mode %s\n", port,
				 (t3 & HOSTPC_PHCD) ? "succeeded" : "failed");
		}
		hal_spin_unlock(&ehci->lock);
	}
	...
	ehci->next_statechange = hal_tick_get();//jiffies + msecs_to_jiffies(10);
...
int ehci_bus_resume(struct usb_hcd *hcd)
{
	...
	while (i--) {
		...
		if ((temp & PORT_PE) && !(temp & (PORT_SUSPEND | PORT_RESUME))) {
	...
	if (ehci->bus_suspended && ehci->has_tdi_phy_lpm) {
		i = HCS_N_PORTS(ehci->hcs_params);
		while (i--) {
			if (usb_test_bit(i, (volatile uint32_t *)&ehci->bus_suspended)) {
				u32 *hostpc_reg = &ehci->regs->hostpc[i];
	...
	while (i--) {
		...
		if (usb_test_bit(i, (volatile uint32_t *)&ehci->bus_suspended) && (temp & PORT_SUSPEND)) {
			temp |= PORT_RESUME;
			usb_set_bit(i, (volatile uint32_t *)&resume_needed);
		}
	...
	while (i--) {
		temp = ehci_readl(ehci, &ehci->regs->port_status[i]);
		if (usb_test_bit(i, (volatile uint32_t *)&resume_needed)) {
			temp &= ~(PORT_RWC_BITS | PORT_SUSPEND | PORT_RESUME);
			ehci_writel(ehci, temp, &ehci->regs->port_status[i]);
		}
	}

	ehci->next_statechange = hal_tick_get();//jiffies + msecs_to_jiffies(5);
	hal_spin_unlock(&ehci->lock);
...
int ehci_bus_suspend(struct usb_hcd *hcd)
{
	hal_log_info("ERR: Not support suspend.\n");
	return 0;
}

int ehci_bus_resume(struct usb_hcd *hcd)
{
	hal_log_info("ERR: Not support resume.\n");
	return 0;
}
...
static void set_owner(struct ehci_hcd *ehci, int portnum, int new_owner)
{
	...
	for (try = 4; try > 0; --try) {
		...
		if ((port_status & PORT_OWNER) == new_owner
		    || (port_status & (PORT_OWNER | PORT_CONNECT)) == 0) {
			try = 0;
		} else {
...
int ehci_hub_status_data(struct usb_hcd *hcd, char *buf)
{
	...
	for (i = 0; i < ports; i++) {
		...
		if ((temp & mask) != 0
		    || usb_test_bit(i, (volatile uint32_t *)&ehci->port_c_suspend)
		    || (ehci->reset_done[i] && time_after_eq(hal_tick_get(), ehci->reset_done[i]))) {
			if (i < 7)
				buf[0] |= 1 << (i + 1);
			else
				buf[1] |= 1 << (i - 7);
	...
	if (ehci->resuming_ports) {
		...
		unsigned long time_interval = MS_TO_OSTICK(25);
		osal_timer_control(hcd->rh_timer, OSAL_TIMER_CTRL_SET_TIME, &time_interval);
	}
...
static void ehci_hub_descriptor(struct ehci_hcd *ehci, struct usb_hub_descriptor *desc)
{
	...
	temp = 0x0008;	// HUB_CHAR_INDV_PORT_OCPM;	/* per-port overcurrent reporting */
	if (HCS_PPC(ehci->hcs_params))
		temp |= 0x0001;	 // HUB_CHAR_INDV_PORT_LPSM; /* per-port power control */
	else
		temp |= 0x0002;	 // HUB_CHAR_NO_LPSM; /* no power switching */
#if 0
...
int ehci_hub_control(struct usb_hcd *hcd,
		     u16 typeReq,
		     u16 wValue,
		     u16 wIndex,
		     char *buf,
		     u16 wLength)
{
	...
	case ClearPortFeature:
		...
		case USB_PORT_FEAT_SUSPEND:
			...
			ehci->reset_done[wIndex] = hal_tick_get()
						   // + MS_TO_OSTICK(USB_RESUME_TIMEOUT);
						   + MS_TO_OSTICK(40);
	...
	case GetPortStatus:
		...
		if (!ehci->reset_done[wIndex]) {
			...
			if (temp & PORT_RESUME) {
				/* resume signaling for 20 msec */
				unsigned long time_interval = MS_TO_OSTICK(20);
				ehci->reset_done[wIndex] = hal_tick_get() + time_interval;
				usb_hcd_start_port_resume(&hcd->self, wIndex);
				usb_set_bit(wIndex, (volatile uint32_t *)&ehci->resuming_ports);
				/* check the port again */
				// mod_timer(&ehci_to_hcd(ehci)->rh_timer,
				//		ehci->reset_done[wIndex]);
				osal_timer_control(ehci_to_hcd(ehci)->rh_timer,
						   OSAL_TIMER_CTRL_SET_TIME, &time_interval);
			}

			/* reset or resume not yet complete */
		} else if (!time_after_eq(hal_tick_get(), ehci->reset_done[wIndex])) {
			; /* wait until it is complete */

			/* resume completed */
		} else if (usb_test_bit(wIndex, (volatile uint32_t *)&ehci->resuming_ports)) {
			usb_clear_bit(wIndex, (volatile uint32_t *)&ehci->suspended_ports);
			usb_set_bit(wIndex, (volatile uint32_t *)&ehci->port_c_suspend);
			...
			usb_clear_bit(wIndex, (volatile uint32_t *)&ehci->resuming_ports);
			retval = ehci_handshake(ehci, status_reg, PORT_RESUME, 0, 2000 /* 2msec */);
			if (retval != 0) {
			 ...
			 */
			retval = ehci_handshake(ehci, status_reg, PORT_RESET, 0, 1000);
			if (retval != 0) {
		...
		} else if (usb_test_bit(wIndex, (volatile uint32_t *)&ehci->suspended_ports)) {
			usb_clear_bit(wIndex, (volatile uint32_t *)&ehci->suspended_ports);
			usb_clear_bit(wIndex, (volatile uint32_t *)&ehci->resuming_ports);
			ehci->reset_done[wIndex] = 0;
			if (temp & PORT_PE)
	...
	case SetPortFeature:
		...
		case USB_PORT_FEAT_RESET:
			...
			if ((temp & (PORT_PE | PORT_CONNECT)) == PORT_CONNECT
				...
				ehci->reset_done[wIndex] = hal_tick_get() + MS_TO_OSTICK(50);
...
__attribute__((__unused__)) static void ehci_relinquish_port(struct usb_hcd *hcd, int portnum) // fix unused-function
{
	struct ehci_hcd *ehci = hcd_to_ehci(hcd);
...
__attribute__((__unused__)) static int ehci_port_handed_over(struct usb_hcd *hcd, int portnum) // fix unused-function
{
	struct ehci_hcd *ehci = hcd_to_ehci(hcd);
	u32 *reg;
