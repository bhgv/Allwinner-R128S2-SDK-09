...
#ifdef CONFIG_COMPONENTS_PM
		sunxi_ohci->hci_dev.ops = &sunxi_ohci_devops;
		sunxi_ohci->hci_dev.name = sunxi_ohci->pm_hci_name;
		sunxi_ohci->hci_dev.data = sunxi_ohci;
		sprintf((char*)sunxi_ohci->hci_dev.name, "sunxi_ohci%d", hci_num);
		pm_devops_register(&sunxi_ohci->hci_dev); 
#endif
		return 0;
	} else {
...
#ifdef CONFIG_COMPONENTS_PM
	pm_devops_unregister(&sunxi_ohci->hci_dev);
#endif
	if (sunxi_ohci->hcd)
