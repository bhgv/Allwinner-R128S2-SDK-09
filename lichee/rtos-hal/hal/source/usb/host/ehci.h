...
struct ehci_per_sched {
	...
	u8 bw_phase;		  /* same, for bandwidth reservation */
	u8 phase_uf;		  /* uframe part of the phase */
	u8 usecs, c_usecs;	  /* times on the HS bus */
	u8 bw_uperiod;		  /* period in microframes, for bandwidth reservation */
...
struct ehci_hcd { /* one per controller */
	...
	unsigned long enabled_hrtimer_events;
	...
	/* list record qtds whitch waiting free
	because OS not support free/malloc at interrupt */
	struct list_head wait_free_list;
	hal_spinlock_t wait_free_list_lock;
...
#define DEFAULT_I_TDPS	1024 /* some HCs can do less */
	...
	unsigned long last_periodic_enable;
	uint32_t command;
 ...
 */
/*
 * After the DMA transfer is completed, because hw_transaction may be changed by hcd,
 * it is necessary to invalidate cache before reading, but the scope must be limited.
 * 36 : hw_next + hw_transaction
 * 92 : hw_next + hw_transaction + hw_bufp + hw_bufp_hi
 */
#define INVALI_ITD_CACHE_RANGE \
    (((CACHELINE_LEN) < 36) ? ((CACHELINE_LEN) * 2) : \
    (((CACHELINE_LEN) <= 92) ? (CACHELINE_LEN) : 0))
struct ehci_itd {
	/* first part defined by EHCI spec */
...
#define ehci_dbg(fmt, args...)		hal_log_debug(fmt, ##args)
#define ehci_err(fmt, args...)		hal_log_err(fmt, ##args)
#define ehci_info(fmt, args...)		hal_log_info(fmt, ##args)
#define ehci_warn(fmt, args...)		hal_log_warn(fmt, ##args)
/*-------------------------------------------------------------------------*/
...
extern void ehci_init_driver(struct hc_driver *drv, const struct ehci_driver_overrides *over);
extern int ehci_setup(struct usb_hcd *hcd);
extern int ehci_handshake(struct ehci_hcd *ehci, u32 *ptr, u32 mask, u32 done, int usec);
extern int ehci_reset(struct ehci_hcd *ehci);

extern int ehci_suspend(struct usb_hcd *hcd, bool do_wakeup);
extern int ehci_resume(struct usb_hcd *hcd, bool force_reset);
extern void ehci_adjust_port_wakeup_flags(struct ehci_hcd *ehci, bool suspending, bool do_wakeup);
