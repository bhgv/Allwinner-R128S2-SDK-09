...
#define dbg_port(hc,label,num,value) \
	hal_log_debug ("%s roothub.portstatus [%d] "  \
...
// #ifdef	CONFIG_PM
__attribute__((__unused__)) static int ohci_rh_suspend(struct ohci_hcd *ohci, int autostop) // fix unused-function
{
	int			status = 0;
	...
	if (ohci->flags & OHCI_QUIRK_GLOBAL_SUSPEND) {
		__hc32 *portstat = ohci->regs->roothub.portstatus;
		int		i;
		unsigned	temp;

		for (i = 0; i < ohci->num_ports; (++i, ++portstat)) {
			temp = ohci_readl(ohci, portstat);
			if ((temp & (RH_PS_PES | RH_PS_PSS)) == RH_PS_PES)
	...
	if (ohci_to_hcd(ohci)->self.root_hub->do_remote_wakeup || autostop) {
		...
		ohci_writel(ohci, OHCI_INTR_RHSC | OHCI_INTR_RD, &ohci->regs->intrdisable);
		ohci->hc_control &= ~OHCI_CTRL_RWE;
	}
	...
	if (!autostop) {
		ohci->next_statechange = hal_tick_get() + MS_TO_OSTICK(5);
		ohci->autostop = 0;
...
static int ohci_rh_resume(struct ohci_hcd *ohci)
{
	...
	hal_msleep(20 /* usb 11.5.1.10 */ + 12 /* 32 msec counter */ + 1);
...
skip_resume:
	...
	ohci->next_statechange = hal_tick_get() + MS_TO_OSTICK(300);
	...
	if (!ohci->ed_rm_list) {
		if (ohci->ed_controltail) {
			ohci_writel(ohci, find_head(ohci->ed_controltail)->dma,
...
// 	 */
// 	rhsc_enable = ohci_readl(ohci, &ohci->regs->intrenable) & OHCI_INTR_RHSC;
...
// 				ohci->autostop = 0;
// 				ohci->next_statechange = jiffies + STATECHANGE_DELAY;
// 			} else if (time_after_eq(jiffies, ohci->next_statechange)
// 				   && !ohci->ed_rm_list
// 				   && !(ohci->hc_control & OHCI_SCHED_ENABLES)) {
...
// 		} else if (!ohci->autostop && !ohci_to_hcd(ohci)->self.root_hub->do_remote_wakeup) {
// 			poll_rh = 0;
 ...
 */
static int ohci_root_hub_state_changes(struct ohci_hcd *ohci,
				       int changed,
				       int any_connected,
				       int rhsc_status)
{
...
int ohci_hub_status_data(struct usb_hcd *hcd, char *buf)
{
	...
	if ((ohci->flags & OHCI_QUIRK_AMD756)
		...
		hal_log_warn("bogus NDP, rereads as NDP=%d\n",
	...
	// else
	// 	usb_clear_bit(HCD_FLAG_POLL_RH, &hcd->flags);

	ohci_root_hub_state_changes(ohci, changed, any_connected, rhsc_status);
...
static inline int root_port_reset(struct ohci_hcd *ohci, unsigned port)
{
	...
	do {
		 ...
		 */
		if (limit_2 < 0) {
			hal_log_debug(ohci, "port[%d] reset timeout, stat %08x\n", port, temp);
			break;
		}
...
int ohci_hub_control(struct usb_hcd *hcd,
		     u16 typeReq,
		     u16 wValue,
		     u16 wIndex,
		     char *buf,
		     u16 wLength)
{
	...
	case ClearPortFeature:
		...
		ohci_writel(ohci, temp, &ohci->regs->roothub.portstatus[wIndex]);
	...
	case SetPortFeature:
		...
		case USB_PORT_FEAT_SUSPEND:
#ifdef CONFIG_USB_OTG
			if (hcd->self.otg_port == (wIndex + 1) && hcd->self.b_hnp_enable)
				ohci->start_hnp(ohci);
			else
#endif
			...
			break;
		case USB_PORT_FEAT_POWER:
			ohci_writel(ohci, RH_PS_PPS, &ohci->regs->roothub.portstatus[wIndex]);
			break;
