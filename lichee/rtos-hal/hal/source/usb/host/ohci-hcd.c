...
int ohci_urb_enqueue(struct usb_hcd *hcd, struct urb *urb, gfp_t mem_flags)
{
	...
	// mem_flags);
	urb_priv = hal_malloc(sizeof(urb_priv_t) + size * sizeof(struct td *));
	...
	if (ed->state == ED_IDLE) {
		...
		if (ohci->prev_frame_no == IO_WATCHDOG_OFF && list_empty(&ohci->eds_in_use)
		...
		if (ed->type == PIPE_ISOCHRONOUS) {
			...
			frame += max((uint16_t)8, (uint16_t)ed->interval);
			frame &= ~(ed->interval - 1);
			frame |= ed->branch;
		...
		if (unlikely(tick_before(frame, next))) {
			...
			if (urb->transfer_flags & URB_ISO_ASAP) {
				frame += (next - frame + ed->interval - 1) & -ed->interval;
			 ...
			 */
				 ...
				 * entirely.
				 */
				urb_priv->td_cnt = DIV_ROUND_UP((u16)(next - frame), ed->interval);
				if (urb_priv->td_cnt >= urb_priv->length) {
					++urb_priv->td_cnt; /* Mark it */
					hal_log_debug("iso underrun %p (%u+%u < %u)\n", urb, frame, length, next);
				}
			}
		}
		urb->start_frame = frame;
...
sanitize:
	...
	case ED_UNLINK: /* wait for hw to finish? */
		...
		if (limit-- == 0) {
			hal_log_warn("ED unlink timeout\n");
			goto sanitize;
		}
		// spin_unlock_irqrestore (&ohci->lock, flags);
		hal_spin_unlock_irqrestore(&ohci->lock, flags);
	...
	default:
		 ...
		 */
		hal_log_err("leak ed %p (#%02x) state %d%s\n", ed, ep->desc.bEndpointAddress,
			    ed->state, list_empty(&ed->td_list) ? "" : " (has tds)");
		td_free(ohci, ed->dummy);
		break;
	}
	ep->hcpriv = NULL;
 ...
 */
__attribute__((__unused__)) static void _ohci_shutdown(struct usb_hcd *hcd) // fix unused-function
{
...
int ohci_init(struct ohci_hcd *ohci)
{
	...
	if (!no_handshake && ohci_readl(ohci, &ohci->regs->control) & OHCI_CTRL_IR) {
		u32 temp;

		hal_log_debug("USB HC TakeOver from BIOS/SMM\n");
		...
		while (ohci_readl(ohci, &ohci->regs->control) & OHCI_CTRL_IR) {
			hal_msleep(10);
			if (--temp == 0) {
				hal_log_err("USB HC takeover failed!"
					"  (BIOS/SMM bug)\n");
				return -EBUSY;
			}
		}
	...
	// ohci->hcca = hal_malloc_coherent (hcd->self.controller,
	// 		sizeof(*ohci->hcca), &ohci->hcca_dma, GFP_KERNEL);
	ohci->hcca = usb_dma_malloc(sizeof(*ohci->hcca), &ohci->hcca_dma, USB_OHCI_HCCA_MALLOC_ALIGN_SIZE);
	if (!ohci->hcca)
		return -ENOMEM;
...
int ohci_run(struct ohci_hcd *ohci)
{
	...
	if (first) {
		...
		if (ohci->fminterval != FI)
			hal_log_debug("fminterval delta %d\n", ohci->fminterval - FI);
	...
	hal_dcache_clean_invalidate((unsigned long)((struct ohci_hcca *)(ohci->hcca_dma)), sizeof(struct ohci_hcca));
...
retry:
	...
	if ((ohci_readl(ohci, &ohci->regs->fminterval) & 0x3fff0000) == 0
		...
		if (!(ohci->flags & OHCI_QUIRK_INITRESET)) {
			...
			hal_log_debug("enabling initreset quirk\n");
			goto retry;
		}
		// spin_unlock_irq (&ohci->lock);
		hal_spin_unlock(&ohci->lock);
	...
	// usb_set_bit(HCD_FLAG_POLL_RH, &hcd->flags);
	...
	ohci_writel(ohci, (val & RH_A_NPS) ? 0 : RH_B_PPCM, &ohci->regs->roothub.b);
	// flush those writes
	(void)ohci_readl(ohci, &ohci->regs->control);
...
// 		head = hc32_to_cpu(ohci, (*(volatile typeof(ed->hwHeadP)*)&(ed->hwHeadP))) & TD_MASK;
// 		td_start = td;
...
hal_irqreturn_t ohci_irq_handler(void *dev)
{
...

{
	if (ints & OHCI_INTR_RHSC) {
		...
		if ((portstatus0 & RH_PS_CCS) && (portstatus0 & RH_PS_CSC)) {
			...
			printf("\n ohci_irq: fullspeed or lowspeed device disconnect\n");
		}
	}
}
	...
	if (ints & OHCI_INTR_UE) {
		...
		if (quirk_nec(ohci)) {
			 ...
			 */
			hal_log_err("OHCI Unrecoverable Error, scheduling NEC chip restart\n");

			ohci_writel(ohci, OHCI_INTR_UE, &regs->intrdisable);

			// schedule_work (&ohci->nec_work);
		} else {
			hal_log_err("OHCI Unrecoverable Error, disabled\n");
			ohci->rh_state = OHCI_RH_HALTED;
			usb_hc_died(hcd);
		}
	...
	if (ints & OHCI_INTR_RHSC) {
		ohci_dbg(ohci, "rhsc\n");
		// ohci->next_statechange = jiffies + STATECHANGE_DELAY;
		ohci_writel(ohci, OHCI_INTR_RD | OHCI_INTR_RHSC, &regs->intrstatus);
...
void ohci_stop(struct usb_hcd *hcd)
{
	...
	if (ohci->hcca) {
		// hal_free_coherent (hcd->self.controller,
		//     sizeof *ohci->hcca,
		//     ohci->hcca, ohci->hcca_dma);
		usb_dma_free(ohci->hcca, ohci->hcca_dma);
		ohci->hcca = NULL;
		ohci->hcca_dma = 0;
	}
}
...
int ohci_resume(struct usb_hcd *hcd, bool hibernated)
{
	...
	if (need_reinit) {
		hal_spin_lock(&ohci->lock);
		ohci_rh_resume(ohci);
		/* FIXME:  ohci_rh_suspend need to be uesd with
		 * usb_hcd_resume_root_hub. But there is no
		 * pm api in usb_hcd_resume_root_hub */
...
// 	usb_set_bit(USB_OHCI_LOADED, &usb_hcds_loaded);
