...
#include "pm_devops.h"
extern struct pm_devops usb_device_pm_ops;
#endif
...
{
	hal_log_info("[%s%d]: remove, sunxi_ehci: 0x%x, 0x%x, irq_no:%u",
		sunxi_ehci->hci_name, sunxi_ehci->usbc_no, sunxi_ehci,
		sunxi_ehci->usb_vbase, sunxi_ehci->irq_no);
...
int sunxi_ehci_hcd_init(int hci_num)
{
	...
	if (hal_cfg_get_keyvalue(ehci_name, KEY_USB_ENABLE, (int32_t *)&value, 1) == 0) {
...
#ifdef CONFIG_COMPONENTS_PM
		sunxi_ehci->hci_dev.ops = &sunxi_ehci_devops;
		sunxi_ehci->hci_dev.name = sunxi_ehci->pm_hci_name;
		sunxi_ehci->hci_dev.data = sunxi_ehci;
		sprintf((char*)sunxi_ehci->hci_dev.name, "sunxi_ehci%d", hci_num);
		pm_devops_register(&sunxi_ehci->hci_dev); 

		sunxi_ehci->usb_dev.ops = &usb_device_pm_ops;
		sunxi_ehci->usb_dev.name = sunxi_ehci->pm_usb_name;
		sunxi_ehci->usb_dev.data = (void *)sunxi_ehci->hcd;
		sprintf((char*)sunxi_ehci->usb_dev.name, "usb%d_device", sunxi_ehci->usbc_no);
		pm_devops_register(&sunxi_ehci->usb_dev);
#endif
		return 0;
	} else {
...
#ifdef CONFIG_COMPONENTS_PM
	pm_devops_unregister(&sunxi_ehci->hci_dev);
	pm_devops_unregister(&sunxi_ehci->usb_dev);
#endif
