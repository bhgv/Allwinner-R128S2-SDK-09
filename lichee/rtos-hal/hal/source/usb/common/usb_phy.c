...
// fix unused-function
__attribute__((__unused__)) static int bit_len(uint32_t mask)
{
	int len = 0;
	int offset = 0;
...
__attribute__((__unused__)) static int bit_val(int value, uint32_t mask)
{
...
// fix unused-function
__attribute__((__unused__)) static int bit_map(int value, uint32_t src_mask, uint32_t dst_mask)
{
...
//fix unused-function
__attribute__((__unused__)) static int bit_read_map(uint64_t phy_addr, uint32_t reg_addr, uint32_t map_mask)
{
...
//fix unused-function
__attribute__((__unused__)) static void bit_cal(int value, uint32_t mask, int *val, int *len)
{
...
__attribute__((__unused__)) static void usb_phy_tp_resistance_soft_adjust(unsigned long phy_vbase, int data)
{
...
//fix unused-function
__attribute__((__unused__)) static int usb_phy_range_register_write(unsigned long phy_vbase, int data)
{
	int val = 0;
...
//fix unused-function
__attribute__((__unused__)) static int usb_phy_range_register_read(unsigned long phy_vbase)
{
	int val = 0;
	int data = 0;
...
{
	if (data > 0x1f) {
		printf("out of range [0x0-0x1f]!\n");
		return -1;
	}

	usb_phy_tp_write(phy_vbase, 0x20, data, 5);
...
static int usb_phy_range_vcbus_read(unsigned long phy_vbase)
{
	uint32_t phy_range, rise, rate;

	rise = usb_phy_tp_read(phy_vbase, 0x20, 2);
	rate = usb_phy_tp_read(phy_vbase, 0x22, 3);
	phy_range = usb_phy_tp_read(phy_vbase, 0x20, 5);
