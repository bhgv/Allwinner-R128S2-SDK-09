...
s32 list_del_node_by_data(void *data, struct usb_list_head *list)
{
	struct usb_list_head *list_now = NULL;
	struct usb_list_head *list_next = NULL;
	struct usb_list_head tmp_list;

	USB_INIT_LIST_HEAD(&tmp_list);
	...
	while (list_now != list) {
		...
		if (list_now->data == data) {
			...
			/* 将其添加到tmp_list，等待释放资源 */
			usb_list_add(list_now, &tmp_list);
			list_now = NULL;
			is_find = 1;
		}
	...
	list_now = tmp_list.next;
	list_next = NULL;

	while (list_now != &tmp_list) {
		list_next = list_now->next;
		usb_list_del_init(list_now);
		/* 释放原来的list */
