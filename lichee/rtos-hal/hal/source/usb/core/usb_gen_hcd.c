...
void usb_hcd_unmap_urb_for_dma(struct usb_hcd *hcd, struct urb *urb)
{
	...
	hal_free_coherent(*vaddr_handle);
			    ...
			    unsigned mem_flags)
{
	...
	vaddr = hal_malloc_coherent(size + CACHELINE_LEN + sizeof(vaddr));
	hal_log_debug("[%s] vaddr:%x, handle:%x/%x, buf:%x/%x, size:0x%x, flag:0x%x, pipe:%x", __func__,
		      vaddr, *vaddr_handle, vaddr_handle, urb->transfer_buffer, urb->transfer_dma, size,
		      urb->transfer_flags, urb->pipe);
	if (!vaddr)
...
void usb_remove_hc_gen_dev(struct usb_hcd *hcd)
{
	...
	//--<4>--释放这个设备所有的资源
	hal_workqueue_cancel_work_sync(hcd->giveback_workqueue, &hcd->giveback_workqueue_task);
...
static int hcd_ops_hub_suspend(struct pm_device *dev, suspend_mode_t mode)
{
	struct usb_hcd *hcd = dev->data;
...
static int hcd_ops_hub_resume(struct pm_device *dev, suspend_mode_t mode)
{
	struct usb_hcd *hcd = dev->data;
...
struct pm_devops usb_device_pm_ops = {
	.suspend = hcd_ops_hub_suspend,
	.resume = hcd_ops_hub_resume,
};
