...
static struct usb_bus my_usb_virt_bus;  // super bus
static hal_mutex_t mutex_lock;
	...
	while (list_start != list_now) {
		...
		if (dev->func_drv == NULL) {
			...
			if (drv->func_drv_ext.probe_ext(dev) == 0) {
				/* device 和 driver 匹配成功, 就将 device 添加到 driver
				 * 的virt_dev_list中*/
...
int32_t usb_virt_bus_dev_add(struct usb_host_virt_sub_dev *dev)
{
	...
	if (dev->func_drv != NULL) {
		...
		while (list_start != list_now) {
			struct usb_driver *func_drv
			    = (struct usb_driver *)list_now->data;
			list_now = list_now->next;
			/* 事先尝试bind到一起 */
			...
			if (func_drv->func_drv_ext.probe_ext(dev) == 0) {
				...
				hal_mutex_lock(mutex_lock);
				list_head_malloc_and_add(dev, &(func_drv->virt_dev_list));
				hal_mutex_unlock(mutex_lock);
...
{
	struct usb_driver *func_driver = NULL;
...
{
	memset(&my_usb_virt_bus, 0, sizeof(struct usb_bus));
	...
	mutex_lock = hal_mutex_create();
	if (!mutex_lock) {
		__err("ERR: usb_virt_bus_init: create mutex lock failed");
		goto ERR0;
	}
