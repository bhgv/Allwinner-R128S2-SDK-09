		...
		struct uvc_buffer *buf, const __u8 *data, int len)
{
	...
	mem = (void *)((size_t)(buf->buf.mem_buf + buf->buf.bytesused));
				  ...
				  struct uvc_buffer *buf)
{
	...
	for (i = 0; i < urb->number_of_packets; ++i) {
		...
		mem = (u8 *)((size_t)(urb->transfer_buffer + urb->iso_frame_desc[i].offset));
		do {
...
int uvc_urb_clear(struct uvc_streaming *stream)
{
    stream->urb_clear = 1;
    while(stream->urb_cleared_count < UVC_URBS) {
        hal_msleep(50);
    }
    stream->urb_clear = 0;
    stream->urb_cleared_count = 0;
    return 0;
}
...
//	stream->decode(urb, stream, buf);
    ...
    if (stream->urb_clear == 1) {
        stream->urb_cleared_count ++;
        return;
    }
...
static void uvc_free_urb_buffers(struct uvc_streaming *stream)
{
	...
	for (i = 0; i < UVC_URBS; ++i) {
		if (stream->urb_buffer[i]) {
			hal_free_align(stream->urb_buffer[i]);
				 ...
				 /*gfp_t*/ __s32 gfp_flags)
{
	...
	for (; npackets >= 1; npackets /= 2) {
		for (i = 0; i < UVC_URBS; ++i) {
			...
			stream->urb_buffer[i] = (char *)hal_malloc_align(stream->urb_size, 64);
#endif
			if (!stream->urb_buffer[i]) {
				hal_log_err("hal_malloc fail\n");
