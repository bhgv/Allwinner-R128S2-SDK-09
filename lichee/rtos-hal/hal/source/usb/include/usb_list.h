 ...
 * data       :  node的内容
 */
#define usb_list_for_each_entry(list_head, list_next, _data)             \
	for ((list_next) = (list_head)->next, (_data) = list_next->data; \
	     (list_next) != (list_head);                                \
	     (list_next) = (list_next)->next, (_data) = list_next->data)
