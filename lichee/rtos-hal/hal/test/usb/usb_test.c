...
static int usb_test_command_otg(int argc, const char **argv)
{
	switch(hal_usb_get_otg_role()) {
	case 0:
		printf("[usb0] is USB NULL\n");
		break;
	case 1:
		printf("[usb0] is USB HOST\n");
		break;
	case 2:
		printf("[usb0] is USB DEVICE\n");
		break;
	}
	return 0;
}
...
static int usb_test_command(int argc, const char **argv)
{
	...
	else if (!strcmp(argv[1], "otg_role"))
		ret = usb_test_command_otg(argc, argv);
