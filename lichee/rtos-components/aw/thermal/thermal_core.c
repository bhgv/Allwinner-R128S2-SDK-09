...
static void thermal_zome_get_temperature(struct thermal_zone_device *tzd, int *temp)
{
	...
	hal_mutex_unlock(tzd->zone->lock);
}
...
static int thermal_zone_set_emul_temp(void *data, const int emul_temp)
{
	...
	hal_mutex_lock(tz->lock);
	tz->emul_temperature = emul_temp;
	hal_mutex_unlock(tz->lock);
							...
							void *priv)
{
	...
	char timer_name[20];
	int32_t crit_temp = THERMAL_TEMP_INVALID;

	if (!zone_name || !ops || !priv)
		return NULL;

#ifdef CONFIG_DRIVER_SYSCONFIG
	if (hal_cfg_get_keyvalue(THERMAL_PARA, "critical_temperature", &crit_temp, 1)) {
		thermal_err("get critical temperature failed\n");
		return NULL;
	}
#endif
	...
	tz->lock = hal_mutex_create();
	hal_mutex_lock(tz->lock);

	tz->temperature = THERMAL_TEMP_INVALID;
	tz->emul_temperature = 0;
	if (crit_temp == THERMAL_TEMP_INVALID)
		crit_temp = THERMAL_DEFAULT_CRITICAL_TEMP;
	tz->crit_temperature = crit_temp;
...
int thermal_zone_device_unregister(struct thermal_zone_device *zone_dev)
{
	...
	hal_mutex_lock(zone_dev->zone->lock);
	if (zone_dev->xHandle)
	...
	hal_mutex_unlock(zone_dev->zone->lock);
	hal_mutex_delete(zone_dev->zone->lock);
	free(zone_dev->zone);
	zone_dev->zone = NULL;
	free(zone_dev);
