...
#ifndef WMG_VERSION
#define WMG_VERSION     "2.0.8.4 20231026 12:00"
#endif
...
/**
 * wifi_sta_auto_connect - Auto connect to an already connected ssid
 *
 * @ssid: the networks want to connect
 * return: WMG_STATUS_FAIL(0) - connect to an already connected ssid success, others - fail
 *         WMG_STATUS_UNSUPPORTED(-5) - Connection failure caused by no connection information
 */
 ...
 * @ssid: config hidden ssid can scan also.
 ...
 */
wmg_status_t wifi_get_scan_results(wifi_scan_result_t *result, char *ssid, uint32_t *bss_num, uint32_t arr_size);
