...
int __wifi_sta_auto_connect(void **call_argv,void **cb_argv)
{
	wifimg_object_t* wifimg_object = get_wifimg_object();
	...
	*cb_status = wifimg_object->sta_auto_connect(ssid);
	if (*cb_status != WMG_STATUS_SUCCESS) {
		WMG_ERROR("wifi sta auto connect fail\n");
	}
...
int __wifi_get_scan_results(void **call_argv,void **cb_argv)
{
	...
	char *ssid = (char *)call_argv[1];
	uint32_t *bss_num = (uint32_t *)call_argv[2];
	uint32_t *arr_size = (uint32_t *)call_argv[3];
	wmg_status_t *cb_status = (wmg_status_t *)cb_argv[0];
	...
	*cb_status = wifimg_object->get_scan_results(scan_results, ssid, bss_num, *arr_size);
	if(*cb_status != WMG_STATUS_SUCCESS) {
...
act_func_t sta_action_table[] = {
	...
	[WMG_STA_ACT_AUTO_CONNECT] = {__wifi_sta_auto_connect, "__wifi_sta_auto_connect"},
...
act_func_t sta_action_table[] = {
	...
	[WMG_STA_ACT_AUTO_CONNECT] = {__wifi_unspport_mode_function, "__wifi_unspport_mode_function"},
...
static wmg_status_t wifimg_sta_auto_connect(char *ssid)
{
	return wifimg_mode_ctl_cmd(STA_MODE_NUM , WMG_STA_CMD_AUTO_CONNECT, ssid, NULL);
}
...
static wmg_status_t wifimg_get_scan_results(wifi_scan_result_t *wifi_scan_results, char *wifi_ssid, uint32_t *wifi_bss_num, uint32_t wifi_arr_size)
{
	wmg_status_t ret;
...
static wifimg_object_t wifimg_object = {
	...
	.sta_auto_connect =  wifimg_sta_auto_connect,
