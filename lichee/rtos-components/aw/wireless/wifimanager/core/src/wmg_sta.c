...
static wmg_status_t sta_busy_change(int time_out)
{
	time_t rawtime;
	struct tm *info;
	...
	if(old_time != 0) {
		if((((info->tm_hour * 3600) + (info->tm_min * 60) + info->tm_sec) - old_time) > time_out) {
			old_time = ((info->tm_hour * 3600) + (info->tm_min * 60) + info->tm_sec);
...
static wmg_status_t sta_connect(void *param,void *cb_msg)
{
	if (sta_object.sta_state == WIFI_STA_CONNECTING ||
		sta_object.sta_state == WIFI_STA_OBTAINING_IP) {
		if(sta_busy_change(300) != WMG_STATUS_SUCCESS) {
			WMG_ERROR("dev busy\n");
			return WMG_STATUS_UNHANDLED;
		} else {
			WMG_WARNG("dev busy time out\n");
			sta_object.sta_state = WIFI_STA_BUSY_TIMEOUT;
		}
	}
...
static wmg_status_t sta_auto_connect(void *param,void *cb_msg)
{
	return sta_object.platform_inf[PLATFORM]->sta_platform_extension(STA_CMD_SET_AUTO_CONN, param, NULL);
}
...
static wmg_status_t sta_mode_ctl(int cmd, void *param, void *cb_msg)
{
	...
	switch (cmd) {
		...
		case WMG_STA_CMD_AUTO_CONNECT:
			return sta_auto_connect(param,cb_msg);
