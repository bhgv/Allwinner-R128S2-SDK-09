...
static int cmd_handle_s(char *p)
{
	...
	if(!strncmp(p, "p2p", 3)) {
		...
		begin = GET_TIME;
        if(strlen(p) > 0)
                ret = wifi_get_scan_results(scan_res, p, &bss_num, scan_max_num);
        else
                ret = wifi_get_scan_results(scan_res, NULL, &bss_num, scan_max_num);
		end = GET_TIME;
		if (ret == WMG_STATUS_SUCCESS) {
			...
			for (i = 0; i < get_scan_results_num; i++) {
				...
				WMG_INFO("[%02d]: bssid=%s  channel=%-3d  freq=%d  rssi=%d  sec=%-15s  ssid=%s\n",
...
#ifdef SUPPORT_STA_MODE
static int get_space_offset(char *p, unsigned m_size)
{
	int i = 0;
	int escape_is_string = 0;
	do {
		if ((p[i] != '\\') && (p[i + 1] == ' ')) {
			break;
		} else if ((p[i] == '\\') && (p[i + 1] == ' ') && (escape_is_string == 1)) {
			break;
		} else {
			if ((p[i] == '\\') && (p[i + 1] == '\\') && (escape_is_string == 0)) {
				escape_is_string = 1;
			} else {
				escape_is_string = 0;
			}
...
static void char_escape(char *dst, char *rst, unsigned m_size)
{
	int i = 0, j = 0;
	int escape_is_string = 0;
	do {
		if ((rst[i] == '\\') && (rst[i+1] == ' ') && (escape_is_string == 0)) {
		} else if ((rst[i] == '\\') && (rst[i+1] == '"') && (escape_is_string == 0)) {
		} else if ((rst[i] == '\\') && (rst[i+1] == '\\') && (escape_is_string == 0)) {
			escape_is_string = 1;
		} else {
			escape_is_string = 0;
...
static int cmd_handle_cM(char *cMp)
{
	WMG_DEBUG("cmd handle cM: %s\n",cMp);
	...
	int begin, end;
#endif
	bool ssid_connect = true;
	char *p = cMp+2;
	char *pch = NULL;
	int ssid_bssid_len = 0;
	int ssid_bssid_total_len = 0;

	int psk_len = 0;
	int psk_total_len = 0;

	char *sec_p = NULL;
	// may be all in spaces
	char ssid_bssid_buf[SSID_MAX_LEN * 2 + 1] = {0};
	char ssid_bssid_temp_buf[SSID_MAX_LEN *2 + 1] = {0};
	char psk_buf[PSK_MAX_LEN *2 + 1] = {0};
	char psk_temp_buf[PSK_MAX_LEN *2 + 1] = {0};
	int ret = -1;
	int i, bss_num = 0, get_scan_results_flag = 0, get_scan_results_num = 0;
	wifi_scan_result_t scan_res[RES_LEN] = {0};
	char scan_res_char[20] = {0};
	char bssid_char[18] = {0};
	char *scan_ssid_p = NULL;
	int scan_flag = 0;
	/* Determine whether the connection uses ssid */
	if(*cMp != 'c') {
		ssid_connect = false;
	}

	int relength = 0;
	ssid_bssid_total_len = get_space_offset(p, strlen(p));
	if (ssid_bssid_total_len > SSID_MAX_LEN * 2) {
		WMG_ERROR("Input ssid is too long\n");
		return -1;
	}
	memcpy(ssid_bssid_temp_buf, p, ssid_bssid_total_len);
	ssid_bssid_temp_buf[ssid_bssid_total_len] = '\0';
	char_escape(ssid_bssid_buf, ssid_bssid_temp_buf, (ssid_bssid_total_len + 1));

	ssid_bssid_len = strlen(ssid_bssid_buf);
	if(ssid_connect) {
		if(ssid_bssid_total_len > SSID_MAX_LEN) {
			WMG_ERROR("Input ssid is longer than %d\n",SSID_MAX_LEN);
			return -1;
		}
		scan_ssid_p = &ssid_bssid_buf;
	} else {
		if(ssid_bssid_total_len > 17) {
			WMG_ERROR("Input bssid is longer than 17\n");
			return -1;
		}
	}

	relength = strlen(p) - ssid_bssid_total_len;
	if (relength <= 0) {
		psk_total_len = 0;
	} else {
		// + 1 for ignoring space
		memcpy(p, (p + ssid_bssid_total_len + 1), relength);
		p[relength] = '\0';

		psk_total_len = get_space_offset(p, strlen(p));
		if (psk_total_len > PSK_MAX_LEN * 2) {
			WMG_ERROR("Input psk is too long\n");
			return -1;
		}
		memcpy(psk_temp_buf, p, psk_total_len);
		psk_temp_buf[psk_total_len] = '\0';
		char_escape(psk_buf, psk_temp_buf, (psk_total_len + 1));
		psk_len = strlen(psk_buf);
		if(psk_len > PSK_MAX_LEN) {
			WMG_ERROR("Input psk is longer than %d\n",PSK_MAX_LEN);
			return -1;
		}
	}
	...
	if(psk_len == 0) {
		...
		cn_para.sec = WIFI_SEC_WEP;
		WMG_INFO("The length of psk is 5, connect to sta use wep(ascii)\n");
	} else {
		if ((psk_len > PSK_MAX_LEN) || (psk_len < 8)) {
			WMG_ERROR("The length of psk(%d) needs to be between 8 and %d\n", psk_len, PSK_MAX_LEN);
			return -1;
		}
		/* juge sec param. */
		relength = strlen(p) - psk_total_len;
		if (relength <= 0) {
			sec_p == NULL;
		} else {
			sec_p = &p[psk_total_len + 1];
		}
		...
		if(sec_p != NULL) {
			...
			} else if(!strncmp(sec_p, "wpa2-sha256", 11)) {
				cn_para.sec = WIFI_SEC_WPA2_PSK_SHA256;
	...
	/* get connect ap info from last scan results.*/
	scan_res->scan_action = false;
	if (scan_flag == 0) {
		scan_flag = 0;
		if(!wifi_get_scan_results(scan_res, scan_ssid_p,  &bss_num, RES_LEN)) {
			get_scan_results_num = (RES_LEN > bss_num ? bss_num : RES_LEN);
			WMG_DEBUG("==The first time scan success, get_scan_results_num %d==\n", get_scan_results_num);
			for (i = 0; i < get_scan_results_num; i++) {
				WMG_DEBUG("==The first time scan success, ssid: %s sec(%s)==\n", scan_res[i].ssid, key_mgmt_to_char(scan_res_char, scan_res[i].key_mgmt));
				memset(scan_res_char, 0, 20);
				if(ssid_connect) {
					if (!strcmp(scan_res[i].ssid, ssid_bssid_buf)) {
						WMG_DEBUG("==The first get scan results success %s = %s sec(%s)==\n", scan_res[i].ssid, ssid_bssid_buf, key_mgmt_to_char(scan_res_char, scan_res[i].key_mgmt));
						get_scan_results_flag = 1;
					}
				} else {
					memset(bssid_char, 0, 18);
					uint8tochar(bssid_char, scan_res[i].bssid);
					if (!strcmp(bssid_char, ssid_bssid_buf)) {
						get_scan_results_flag = 1;
					}
				}
			...
			WMG_ERROR("==The first time scan action failed!==\n");
		}
	}

	/* get connect ap info failed from last scan results, scan first before connect. */
	if (get_scan_results_flag == 0){
		...
		if (scan_flag == 0) {
			...
			if(!wifi_get_scan_results(scan_res, scan_ssid_p,  &bss_num, RES_LEN)) {
				get_scan_results_num = (RES_LEN > bss_num ? bss_num : RES_LEN);
				get_scan_results_flag = 0;
				WMG_DEBUG("==The second time scan success, get_scan_results_num %d==\n", get_scan_results_num);
				for (i = 0; i < get_scan_results_num; i++) {
					WMG_DEBUG("==The second time scan success, ssid: %s sec(%s)==\n", scan_res[i].ssid, key_mgmt_to_char(scan_res_char, scan_res[i].key_mgmt));
					memset(scan_res_char, 0, 20);
					if(ssid_connect) {
						if (!strcmp(scan_res[i].ssid, ssid_bssid_buf)) {
							WMG_DEBUG("==The second get scan results success %s = %s sec(%s)==\n", scan_res[i].ssid, ssid_bssid_buf, key_mgmt_to_char(scan_res_char, scan_res[i].key_mgmt));
							get_scan_results_flag = 1;
						}
					} else {
						memset(bssid_char, 0, 18);
				...
				WMG_ERROR("==The second time scan action failed!==\n");
			}
		}
	}
    ...
    if(get_scan_results_flag) {
		if(ssid_connect)
			WMG_DEBUG("==Wi-Fi scan ssid: %s (sec%s)==\n", ssid_bssid_buf, key_mgmt_to_char(scan_res_char, cn_para.sec));
        else
			WMG_DEBUG("==Wi-Fi scan bssid: %s (sec%s)==\n", ssid_bssid_buf, key_mgmt_to_char(scan_res_char,cn_para.sec));
    } else {
        WMG_WARNG("==Wi-Fi can't scan ssid or bssid :%s==\n", ssid_bssid_buf);
        WMG_WARNG("==scan_results: %d, scan_max_num: %d==\n", bss_num, RES_LEN);
        ret = -1;
        goto results;
    }
	...
	if (wmg_state.current_mode & WIFI_STATION) {
		if(wmg_state.sta_state == WIFI_STA_NET_CONNECTED) {
			...
			if(ssid_connect) {
				if(!strcmp(wifi_sta_info.ssid, ssid_bssid_buf)) {
					WMG_INFO("==wifi already connect to %s,and not need to change==\n",wifi_sta_info.ssid);
					return 0;
				}
				WMG_INFO("==wifi already connect to %s,but now need to change to connect%s.==\n", wifi_sta_info.ssid, ssid_bssid_buf);
			} else {
				memset(bssid_char, 0, 18);
				uint8tochar(bssid_char, wifi_sta_info.bssid);
				if (!strcmp(bssid_char, ssid_bssid_buf)) {
					WMG_INFO("==wifi already connect to bssid %s,and not need to change==\n",bssid_char);
					return 0;
				}
				WMG_INFO("==wifi already connect to bssid %s,but now need to change to connect%s.==\n", bssid_char, ssid_bssid_buf);
			}
		}
	}

	/* get ssid, psk, sec, connecting. */
	cn_para.password = psk_buf;
	cn_para.fast_connect = 0;
	if(ssid_connect) {
		WMG_INFO("connect to sta(%s) with pask(%s)\n", ssid_bssid_buf, psk_buf);
		cn_para.ssid = ssid_bssid_buf;
	} else {
		cn_para.ssid = NULL;
		WMG_INFO("connect to sta(bssid: %s) with pask(%s)\n", ssid_bssid_buf, psk_buf);
		pch = strtok(ssid_bssid_buf, ":");
		for(i = 0;(pch != NULL) && (i < 6); i++){
			cn_para.bssid[i] = char2uint8(pch);
			pch = strtok(NULL, ":");
		}
	}
	ret = wifi_sta_connect(&cn_para);
...
static int cmd_handle_t(char *p)
{
	WMG_DEBUG("cmd handle t: %s\n",p);
	...
	int ret = -1;
	char ssid_buf[SSID_MAX_LEN * 2 + 1] = {0};
	char ssid_tmp_buf[SSID_MAX_LEN *2 + 1] = {0};
	int ssid_len = 0;
	int ssid_total_len = 0;

	ssid_total_len = get_space_offset(p, strlen(p));
	if (ssid_total_len > SSID_MAX_LEN * 2) {
		WMG_ERROR("Input ssid is too long\n");
		return -1;
	}

	memcpy(ssid_tmp_buf, p, ssid_total_len);
	ssid_tmp_buf[ssid_total_len] = '\0';
	char_escape(ssid_buf, ssid_tmp_buf, (ssid_total_len + 1));

	begin = GET_TIME;
	ret = wifi_sta_auto_connect(ssid_buf);
	end = GET_TIME;
	WMG_INFO("==Wi-Fi sta auto connect(%s) %s,time %4f ms==\n", ssid_buf, (ret == 0)?"success":"failed", USE_TIME(begin, end));
	return ret;
}
#endif
...
results:
	if(ret) {
		WMG_ERROR("get ap/p2p config failed\n");
		return WMG_STATUS_FAIL;
	} else {
		WMG_DEBUG("get ap/p2p config successful\n");
...
static int cmd_handle_p(char *p)
{
	...
	cn_para.fast_connect = 0;
	cn_para.sec = WIFI_SEC_WPA2_PSK;
	if (!wifi_get_scan_results(scan_res, cn_para.ssid, &bss_num, RES_LEN)) {
		get_scan_results_num = (RES_LEN > bss_num ? bss_num : RES_LEN);
		get_scan_results_flag = 0;
		for (i = 0; i < get_scan_results_num; i++) {
			if (!strcmp(scan_res[i].ssid, cn_para.ssid)) {
				get_scan_results_flag = 1;
			}
			if(get_scan_results_flag) {
				cn_para.sec = WIFI_SEC_NONE;
...
int main(void)
{
	...
	wmg_set_debug_level(DEFAULT_DEBUG_LEVEL);
