...
static void le_conn_phy_update(struct bt_conn *conn)
{
    int err;
    ...
    if (err) {
        BTMG_ERROR("LE PHY update failed (err %d)", err);
    } else {
        BTMG_INFO("LE PHY update initiated.");
    }
}
#endif
...
void le_conn_data_len_update(struct bt_conn *conn)
{
    int err;
    uint8_t phy = BT_GAP_LE_PHY_2M;
    struct bt_conn_le_data_len_param param;

    if (!conn) {
        BTMG_ERROR("Not Device connected");
        return;
    }
    ...
    if (err) {
        BTMG_ERROR("data len update failed (err %d).", err);
    } else {
        BTMG_INFO( "data len update initiated.");
    }
}
#endif
static void le_connected(struct bt_conn *conn, uint8_t err)
{
    ...
    BTMG_DEBUG("own->role: %s", conn->role == 0? "gattc": "gatts");
    if (conn->role == 0) {
#if defined(CONFIG_BT_USER_DATA_LEN_UPDATE)
        le_conn_data_len_update(conn);
#endif

#if defined(CONFIG_BT_USER_PHY_UPDATE)
        le_conn_phy_update(conn);
#endif
    }
