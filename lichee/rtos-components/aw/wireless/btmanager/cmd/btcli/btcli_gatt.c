...
static uint8_t gatts_rate_read = 0;
...
uint64_t throughput_cal_time(uint64_t expect_time_ms)
{
    static int first_flag = 1;
    static uint64_t start_time = 0;
    uint64_t cur_time = hal_gettime_ns();
    uint64_t t = 0;

    t = (cur_time - start_time + t) / 1000000;
    if (first_flag == 1) {
        start_time = cur_time;
        first_flag = 0;
    }
...
void btcli_gattc_notify_indicate_cb(gattc_notify_indicate_cb_para_t *data)
{
    ...
    if (data->length > 255) {
        CMD_DBG("data->length is too large, intercept 255 bytes\n");
        data->length = 255;
    }
...
void btcli_gatts_char_write_req_cb(gatts_char_write_req_t *data)
{
    if (gatts_rate_read) {
        ...
        data_count += data->value_len;
        time_ms = throughput_cal_time(1000);
        if (time_ms) {
            speed = (data_count * 1000/ time_ms) / 1000.0f;
            printf("BLE RX Rate:[%.2f KB/s]\n", speed);
            data_count = 0;
        }
    } else {
        char recv_data[256];

        if (data->value_len > 255) {
            CMD_DBG("data->value_len is too large, intercept 255 bytes\n");
            data->value_len = 255;
        }
...
int btcli_gatt_register_test_service(void)
{
    ...
    prop = BTMG_GATT_CHRC_READ | BTMG_GATT_CHRC_WRITE_WITHOUT_RESP;
...
enum cmd_status btcli_gattc_rate_write(char *cmd)
{
    int argc;
    char *argv[3];
    int err;
    uint16_t byte_size = 244;
    uint16_t loops = 1000;
    uint16_t handle = 0;
    uint16_t conn_id = 0;
    uint16_t att_mtu = 0;
    ...
    att_mtu = btmg_le_conn_get_mtu(conn_id);

    CMD_DBG("ATT MTU: %d\n", att_mtu);
    int send_max = att_mtu -3;

    if (byte_size > send_max) {
        byte_size = send_max;
    }

    CMD_DBG("BLE Send byte_size: %d, loops: %d \n", byte_size, loops);

    for (int i = 0; i < loops; i++) {
        err = btmg_gattc_write_without_response(conn_id, false, handle, gatt_write_buf, byte_size);
        if (err) {
            CMD_ERR("Write failed (err %d)\n", err);
            break;
        }
        ...
        data_count += byte_size;
        time_ms = throughput_cal_time(1000);
        if (time_ms) {
            speed = (data_count * 1000/ time_ms) / 1000.0f;
            printf("BLE TX Rate:[%.2f KB/s]\n", speed);
            data_count = 0;
        }
    }
...
enum cmd_status btcli_gatts_rate_read(char *cmd)
{
    int argc;
    char *argv[1];
...
static const struct cmd_data gatt_cmds[] = {
    ...
    { "rate_write",     btcli_gattc_rate_write,   CMD_DESC("<handle> <len> [loop]")},
    { "rate_read",      btcli_gatts_rate_read,    CMD_DESC("<1 or 0>")},
