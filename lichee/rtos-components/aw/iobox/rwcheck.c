...
#define DEFAULT_TMP_DIR "rwcheck_tmp_dir"
#define DEFAULT_FILES_PER_DIR (1024)
...
static uint64_t get_test_size(struct rwtask *task)
{
    ...
    printf("\tdisc free space:%lld M, min free:%lld M\n", free_bytes / 1000000, task->min_free / 1000000);
...
{
    uint32_t dir_idx = index / DEFAULT_FILES_PER_DIR;

    memset(task->path, 0, MAX_PATH_LEN);
    snprintf(task->path, MAX_PATH_LEN, "%s/%s%d", task->dir,
             DEFAULT_TMP_DIR, dir_idx);
    if ((index % DEFAULT_FILES_PER_DIR) == 0) { /* next dir */
        if (access(task->path, R_OK)) {
            if (mkdir(task->path, 0777))
                printf("mkdir %s failed\n", task->path);
        }
    }
    snprintf(task->path, MAX_PATH_LEN, "%s/%s%d/%s%d.%d", task->dir,
             DEFAULT_TMP_DIR, dir_idx,
