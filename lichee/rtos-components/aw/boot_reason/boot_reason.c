...
const char* boot_reason_to_desc_str(uint32_t boot_reason)
{
	...
	case SUNXI_BOOT_REASON_DSP_WATCHDOG_RESET:
		...
		break;
	case SUNXI_BOOT_REASON_HIBERNATION_WAKEUP_IO:
		str = ENUM_VALUE_TO_STR(SUNXI_BOOT_REASON_HIBERNATION_WAKEUP_IO);
		break;
	case SUNXI_BOOT_REASON_HIBERNATION_RTC_ALARM:
		str = ENUM_VALUE_TO_STR(SUNXI_BOOT_REASON_HIBERNATION_RTC_ALARM);
		break;
	default:
		str = "unknown";
		break;
