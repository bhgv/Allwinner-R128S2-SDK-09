...
int hal_ringbuffer_get(hal_ringbuffer_t rb, void *buf, int size, unsigned int timeout)
{
	...
	if (rb->isfull != true && rb->end - rb->start == 0 && timeout != 0) {
		int ret;
		hal_event_clear_bits(rb->event, RB_EV_WRITE);
		hal_mutex_unlock(rb->mutex);
		ret = hal_event_wait(rb->event, RB_EV_WRITE,
...
int hal_ringbuffer_wait_put(hal_ringbuffer_t rb, const void *buf, int size, int timeout)
{
	...
	if (rb->isfull ||
		...
		int ret;
		rb->threshold = size;
		hal_event_clear_bits(rb->event, RB_EV_READ_AVAIL);
		hal_mutex_unlock(rb->mutex);
		ret = hal_event_wait(rb->event, RB_EV_READ_AVAIL,
					HAL_EVENT_OPTION_CLEAR | HAL_EVENT_OPTION_AND, timeout);
		if (!(ret & RB_EV_READ_AVAIL))
			return HAL_TIMEOUT;
		hal_mutex_lock(rb->mutex);
	}
	hal_mutex_unlock(rb->mutex);
