...
struct pm_task *pm_task_check_inarray(TaskHandle_t xHandle)
{
	int i = 0;
        //uint32_t flags;
...
int pm_task_freeze(pm_task_type_t type)
{
	...
	case PM_TASK_TYPE_APP:
		...
		break;
	case PM_TASK_TYPE_FREEZE_AT_ONCE:
		for (i = 0; i < PM_TASK_MAX; i++) {
			ptr = &pm_task_array[i];
			if (ptr->status && (ptr->type == type)) {
				if (ptr->xHandle == xTaskGetCurrentTaskHandle()) {
					pm_dbg("skip current pm task: %s(%p)\n",\
						pcTaskGetName(ptr->xHandle), ptr->xHandle);
					continue;
				}
				pm_dbg("Task %s(%p) suspend.\n",\
