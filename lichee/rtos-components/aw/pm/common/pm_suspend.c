...
static void pm_suspend_task(void *nouse)
{
	...
	while (1) {
		...
		if (ret)
			pm_notify_event(mode, PM_EVENT_SUSPEND_FAIL);
#endif
...
label_ops_end:
	if (virt_log_stage == PM_VIRT_LOG_EARLY_STAGE) {
		pm_log("restore from virtual log\n");
		pm_virt_log(0);
	}
...
label_restore_app:
	...
	trigger_task_flag = !(!!trigger_task_flag);
	pm_task_restore(PM_TASK_TYPE_FREEZE_AT_ONCE);
...
static int pm_suspend_enter(suspend_mode_t mode)
{
	...
	pm_moment_record(PM_MOMENT_SUSPEND_NOIRQ_END, 0);
	if (ret)
	...
	if (virt_log_stage == PM_VIRT_LOG_LATE_STAGE) {
		pm_log("switch from virtual log\n");
		pm_virt_log(1);
	}
...
lable_dev_resume_noirq:
	...
	pm_moment_record(PM_MOMENT_RESUME_NOIRQ_BEGIN, 0);
