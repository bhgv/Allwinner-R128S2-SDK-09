...
static int wupio_irq = -1;
static int wupio_set = 0;
static struct pm_wakesrc *wupio_ws = NULL;
static hal_irqreturn_t wupio_callback(void *data)
{
	volatile u32 val = 0x0;

	pm_wakecnt_inc(wupio_ws);
...
int pm_set_wupio(uint32_t val, uint32_t edge_mode, uint32_t time_s)
{
	...
	wupio_irq = pmws->irq;
	if (!wupio_set) {
		wupio_ws = pm_wakesrc_register(wupio_irq, "WUPIO", PM_WAKESRC_ALWAYS_WAKEUP);
		if (!wupio_ws) {
			printf("wupio ws register failed\n");
			return -EINVAL;
		}

		ret = pm_set_wakeirq(wupio_ws);
		if (ret)
			printf("wupio ws enable failed\n");
	 ...
	 * clk = LFCLK / 2^clk_sel
	 * cycle = clk * T_s
	 * cycle range 0-15, and the time step is 1 / clk.
	 */
	if ((time_s > 1) && (time_s < 3)) {
		...
		pm_gprcm_set_wup_io_dbc(val, WUP_IO_DBC_FAST_MODE, 0xd, cycle);
		printf("debounce cycle: %d\n", cycle);
	} else {
		/* the wakeup threshold is closed to 200ms */
		pm_gprcm_set_wup_io_dbc(val, WUP_IO_DBC_FAST_MODE, 0xd, 0);
	}
...
int pm_clear_wupio(uint32_t val)
{
	...
	wupio_set &= ~(0x1 << val);
	if (!wupio_set) {
		writel(0x0, PM_RTC_IO_WAKE_GEN);
		hal_disable_irq(wupio_irq);
		hal_free_irq(wupio_irq);

		ret = pm_clear_wakeirq(wupio_ws);
		if (ret)
			printf("wupio ws clear failed\n");

		ret = pm_wakesrc_unregister(wupio_ws);
		if (ret)
			printf("wupio ws unregister failed\n");

		printf("wupio unregister irq\n");
		wupio_irq = -1;
	}
