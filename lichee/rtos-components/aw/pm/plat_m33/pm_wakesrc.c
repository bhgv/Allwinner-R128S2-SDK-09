...
#define SUN20IW2_PMU_BASE		(0x40051400)
#define PMU_RTC_IO_WAKE_EN		(SUN20IW2_PMU_BASE + 0x110)
#define PMU_RTC_IO_WAKE_ST		(SUN20IW2_PMU_BASE + 0x118)
#define PMU_RTC_IO_HOLD_CTRL		(SUN20IW2_PMU_BASE + 0x11c)
#define PMU_RTC_IO_WAKE_GEN		(SUN20IW2_PMU_BASE + 0x120)
...
{
	writel(0x0, PMU_RTC_IO_HOLD_CTRL);
	writel(0x0, PMU_RTC_IO_WAKE_EN);
	writel(0x0, PMU_RTC_IO_WAKE_GEN);
	writel(readl(PMU_RTC_IO_WAKE_ST), PMU_RTC_IO_WAKE_ST);
}
...
#if 0
/*
 * wakecnt - wakeup event count
 * The event is espected to be reported by the module itself.
 * The framework is responsible for suspend abortion.
 */
				pm_wakecnt_inc(irq);
#endif
			}
		...
		if (!hal_interrupt_get_nest() && (pm_state_get() != PM_STATUS_RUNNING)) {
			if (ws && (pm_wakeup_irq == INVALID_IRQn)) {
				pm_wakeup_irq = irq;
				wakesrc_event = id_sum & wakesrc_active;
			}
