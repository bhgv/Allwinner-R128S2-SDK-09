...
//	sr32(VDD_APP_PWROFF_GATING,2,1,0);//poch
	...
	if (((para->dram_tpr7>>0) & 0x1) == 0) {
		...
		sr32(DEV_CLK_CTRL, 21, 1, 1);
		// CCMU_AON DPLL LS clk
		sr32(DPLL1_OUT_CTRL, 15, 1, 0);
		sr32(DPLL3_OUT_CTRL, 15, 1, 1);
	}
...
//	}
	sr32(HSPSRAM_CLKCFG,31,1,0);	//disable the hspsram clk
...
void mc_init(__dram_para_t *para)
{
	...
	if (para->dram_type == 0xb){
		...
		put_wvalue(PREF_CFG1,0x00060000);
...
//	hspsram_sim_delay(10000000);
	...
	dram_dbg_0("DRAM BOOT DRIVE INFO 001: %s\n", VERSION);
