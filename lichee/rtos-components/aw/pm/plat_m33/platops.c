...
int pm_set_time_to_wakeup_ms(unsigned int ms)
{
	...
	if ((!wuptimer_has_irq) && (time_to_wakeup_ms != 0)) {
		...
		pmu_timer_ws = pm_wakesrc_register(AR200A_WUP_IRQn, "time_to_wakeup_ms", PM_WAKESRC_ALWAYS_WAKEUP);
		if (!pmu_timer_ws) {
...
static int pm_m33_enter(suspend_mode_t mode)
{
	...
	if (mode == PM_MODE_HIBERNATION) {
		if (chipv > SUN20I_CHIPV_B)
			...
			HAL_PRCM_SetRTCLDOSlpVolt(PRCM_RTC_LDO_SLEEP_VOLT_875MV);
	} else if (mode == PM_MODE_STANDBY) {
		HAL_PRCM_SetRTCLDOActVolt(PRCM_RTC_LDO_ACTIVE_VOLT_975MV);
		HAL_PRCM_SetAONLDOSlpVoltage(1075);
	}
