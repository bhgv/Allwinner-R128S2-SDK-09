...
/**
 * Deinitialization of settings and backend
 *
 * Called when ble host deinit.
 * Could just deinit backend and FS is not unmount
 *
 * @return 0 on success, non-zero on failure.
 */
...
/**
 * Unregister a handler for settings items stored in RAM.
 *
 * @param handler where handler unregister from.
 *
 * @return 0 on success, non-zero on failure.
 */
...
#if defined(CONFIG_BT_DEINIT)
/**
 * Unregister a backend handler acting as source.
 *
 * @param cs Backend handler node containing handler information.
 *
 */
...
/**
 * Unregister a backend handler acting as destination.
 *
 * @param NULL
 *
 */
