...
static inline void adv_send(struct net_buf *buf)
{
	...
	/*ble controller decrease the sending frequency of adv packets by 4 times
	when advertise and scan coexist. The reason is not clear, and it will make
	advetise packet sent out only 1/4 of the original. In order to keep a
	sufficient number of broadcase packets sent out during the adv period, the
	ble host will increase the advertise frequency by 4 times. This is a
	temporary bypass and wait for the ble controller problem is solved, this
	can be deleted. By liuzhang, 20200810*/

	/*Now ble scan is greater than befor.Don't need it now.*/
#if 0
	duration = duration * 4;
#endif
	/* When mesh and wlan coexist, adv packets will be reduced a lot.
	 * Increase the advertise duration by 2 times,
	 * to ensure enough packets during the adv period.
	 */
	/* TODO: it only take effect when wlancoex active */
	duration = duration * 2;
#endif
	...
	BT_DBG("Advertising stopped (%u ms)", (uint32_t) k_uptime_delta((int64_t *)&time));
}
...
void bt_mesh_adv_init(void)
{
	k_thread_create(&adv_thread_data, adv_thread_stack,
			...
			NULL, NULL, NULL, K_PRIO_COOP(CONFIG_BT_MESH_ADV_THREAD_PRIO), 0, K_FOREVER);
