...
static void friend_clear(struct bt_mesh_friend *frnd)
{
	...
	/*
	 * When friend on, lpn would perform friend request.
	 * As exist old friend information, it need to clear friendship.
	 * Before receiving the friend clear cfm, the friend clear request will be repeated until the clear timeout.
	 * If this happens, clear_timeout() would be executed, which will access the frnd structure of the null pointer.
	 */
