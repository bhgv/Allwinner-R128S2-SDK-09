...
#if defined(CONFIG_BT_MESH) && !defined(CONFIG_USE_SETTINGS_WITHOUT_ZEPHYR)
extern int bt_mesh_settings_init_get(void);
#endif
#endif
...
#if defined(CONFIG_SETTINGS_DYNAMIC_HANDLERS)
	sys_slist_deinit(&settings_handlers);
#endif /* CONFIG_SETTINGS_DYNAMIC_HANDLERS */
	settings_store_deinit();

	k_mutex_unlock(&settings_lock);

	err = k_mutex_delete(&settings_lock);
	if (err) {
		return err;
	}
...
int settings_unregister(struct settings_handler *handler)
{
	...
	SYS_SLIST_FOR_EACH_CONTAINER(&settings_handlers, ch, node) {
		if (strcmp(handler->name, ch->name) == 0) {
			sys_slist_find_and_remove(&settings_handlers, &handler->node);
		}
	}
...
#if (CONFIG_BLEHOST_Z_ITERABLE_SECTION || CONFIG_SETTINGS_DYNAMIC_HANDLERS)
	const char *tmpnext;
#endif
			      ...
			      const struct settings_load_arg *load_arg)
{
	...
	if (load_arg && load_arg->cb) {
...
#if defined(CONFIG_BLEHOST_Z_ITERABLE_SECTION)
#if defined(CONFIG_BT_MESH) && !defined(CONFIG_USE_SETTINGS_WITHOUT_ZEPHYR)
		if (!bt_mesh_settings_init_get() && !strncmp(name, "bt/mesh", 7)) {
			return 0;
		}
#endif
#endif
		...
		if (rc != 0) {
			BT_ERR("set-value failure. key: %s error(%d)",
				log_strdup(name), rc);
			/* Ignoring the error */
			rc = 0;
		} else {
			BT_DBG("set-value OK. key: %s",
				log_strdup(name));
		}
	}
	return rc;
...
#if defined(CONFIG_BT_MESH) && !defined(CONFIG_USE_SETTINGS_WITHOUT_ZEPHYR)
		if (!bt_mesh_settings_init_get() && !strncmp(ch->name, "bt/mesh", 7)) {
			continue;
		}
#endif
		if (ch->h_commit) {
