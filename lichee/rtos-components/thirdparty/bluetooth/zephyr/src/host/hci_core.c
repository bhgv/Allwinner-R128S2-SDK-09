...
//#include <soc.h> //Wewisetek Modified
...
#ifdef CONFIG_BT_DRIVERS_LOG_EXTRA_STACK_SIZE
#define EXTRA_TX_STACK CONFIG_BT_DRIVERS_LOG_EXTRA_STACK_SIZE
#else
#define EXTRA_TX_STACK (0)
#endif
...
static K_KERNEL_STACK_DEFINE(tx_thread_stack, CONFIG_BT_HCI_TX_STACK_SIZE + EXTRA_TX_STACK);

#if defined(CONFIG_BT_DEINIT)
struct k_sem deinit_disconn_sem;
struct bt_conn *deinit_disconn_conn;
#endif
...
/* add lock to avoid two continuous advertise on(off) command send to Controller */
static struct k_mutex adv_lock;

#if defined(CONFIG_BT_OBSERVER)
/* add lock to avoid two continuous scan on(off) command send to Controller */
...
#ifdef CONFIG_BT_XR_IMPL
/* When adv and scan are enabled together, there may be problems with task synchronization.
 * As adv executed by work_q task, and scan executed by console task, it may happen that
 * adv has setted the device random address but scan don't know, which will cause scan
 * setting random address during adv enable, that is disallowed.
 */
static struct k_mutex rand_addr_lock;
#endif
...
#else
static struct bt_le_ext_adv *adv_pool;
#endif

#if defined(CONFIG_BT_VAR_MEM_DYNC_ALLOC)
void adv_ext_dynamic_mem_alloc(void)
{
	adv_pool = (struct bt_le_ext_adv *)k_malloc(sizeof(struct bt_le_ext_adv) * CONFIG_BT_EXT_ADV_MAX_ADV_SET);
	if (adv_pool == NULL) {
		BT_ERR("Alloc adv pool error");
		return ;
	}
	memset(adv_pool, 0, sizeof(struct bt_le_ext_adv) * CONFIG_BT_EXT_ADV_MAX_ADV_SET);
}

void adv_ext_dynamic_mem_free(void)
{
	k_free(adv_pool);
	adv_pool = NULL;
}
...
uint16_t bt_hci_get_sent_cmd_opcode(void)
{
	if (bt_dev.sent_cmd == NULL)
		return -1;
	return cmd(bt_dev.sent_cmd)->opcode;
}
#endif
...
#if defined(CONFIG_BT_VAR_MEM_DYNC_ALLOC)
	__ASSERT(0 <= index && index < CONFIG_BT_EXT_ADV_MAX_ADV_SET,
...
#if defined(CONFIG_BT_VAR_MEM_DYNC_ALLOC)
	for (i = 0; i < CONFIG_BT_EXT_ADV_MAX_ADV_SET; i++) {
#else
...
{
#if defined(CONFIG_BT_VAR_MEM_DYNC_ALLOC)
	if (handle < CONFIG_BT_EXT_ADV_MAX_ADV_SET &&
#else
...
#ifdef CONFIG_BT_XR_IMPL
	k_mutex_lock(&rand_addr_lock, K_FOREVER);
#endif

	/* check if RPA is valid */
	if (atomic_test_bit(bt_dev.flags, BT_DEV_RPA_VALID)) {
		//printf("*************************************************\n");
		//printf("[RandomAddress %s ]\n", bt_addr_str(&bt_dev.random_addr.a));
		//printf("*************************************************\n");
#ifdef CONFIG_BT_XR_IMPL
		k_mutex_unlock(&rand_addr_lock);
#endif
		return 0;
	}
...
static void le_update_private_addr(void)
{
	...
	/*
	 * Core v5.1: If the Host issues this command when any of scanning, initiating, or legacy
	 * advertising are enabled, the Controller shall return the error code Command
	 * Disallowed (0x0C).
	 */
...
#else
		BT_DBG("Unable to find conn for handle %u", acl(buf)->handle);
#endif
		net_buf_unref(buf);
		return;
	}
...
#else
			BT_DBG("No connection for handle %u", handle);
#endif
			continue;
		}
...
#if 1//CONFIG_LTK_DEBUG
		printf("ltk: %02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x\n",
...
#if defined(CONFIG_BT_DEINIT)
#define DEINIT_SIGNAL_COUNT 1
#else
#define DEINIT_SIGNAL_COUNT 0
#endif

#if defined(CONFIG_BT_CONN)
#if defined(CONFIG_BT_ISO)
/* command FIFO + conn_change signal + MAX_CONN + MAX_ISO_CONN */
#define EV_COUNT (2 + CONFIG_BT_MAX_CONN + CONFIG_BT_ISO_MAX_CHAN + DEINIT_SIGNAL_COUNT)
#else
/* command FIFO + conn_change signal + MAX_CONN */
#define EV_COUNT (2 + CONFIG_BT_MAX_CONN + DEINIT_SIGNAL_COUNT)
#endif /* CONFIG_BT_ISO */
#else
/* command FIFO */
#define EV_COUNT (1 + DEINIT_SIGNAL_COUNT)
#endif

#if defined(CONFIG_BT_DEINIT)
static struct k_poll_signal tx_deinit =
		K_POLL_SIGNAL_INITIALIZER(tx_deinit);

void tx_deinit_poll_event_init(struct k_poll_event *events)
{
	tx_deinit.signaled = 0U;
	k_poll_event_init(events, K_POLL_TYPE_SIGNAL,
			  K_POLL_MODE_NOTIFY_ONLY, &tx_deinit);
}
#endif
...
#if defined(CONFIG_BT_DEINIT)
		tx_deinit_poll_event_init(&events[ev_count]);
		ev_count++;
#endif
...
#if defined(CONFIG_BT_DEINIT)
		if (events[ev_count - 1].state & K_POLL_STATE_SIGNALED ) {
#ifdef CONFIG_COMPONENTS_BT_PM
			bt_pm_unlock();
#endif
			goto exit;
...
#if defined(CONFIG_BT_DEINIT)
exit:
	while ((buf = net_buf_get(&bt_dev.cmd_tx_queue, K_NO_WAIT))) {
		hci_cmd_done(cmd(buf)->opcode, BT_HCI_ERR_SUCCESS, buf);
		net_buf_unref(buf);
	}
...
int bt_hci_tx_thread_deinit(void)
{
	...
	k_poll_signal_raise(&tx_deinit, 1); /* same as k_poll_signal */

	k_thread_join(&tx_thread_data, K_FOREVER);

	tx_deinit = (struct k_poll_signal)K_POOL_SIGNAL_DEINITIALIZER(tx_deinit);
...
static int le_init(void)
{
	...
	k_mutex_init(&adv_lock);
#if defined(CONFIG_BT_OBSERVER)
	k_mutex_init(&scan_lock);
#endif

#ifdef CONFIG_BT_XR_IMPL
	k_mutex_init(&rand_addr_lock);
#endif
...
{
	static const char * const xradiotech_str[] = {
		"reserved", "XR806"
	};
...
#if defined(CONFIG_BT_DEINIT)
int bt_hci_driver_unregister(const struct bt_hci_driver *drv)
{
	if (!bt_dev.drv) {
		return -ENODEV;
	}
	...
	BT_DBG("Unregistered %s", drv->name ? drv->name : "bt0");
...
#if defined(CONFIG_BT_DEINIT)
exit:
	while ((buf = net_buf_get(&bt_dev.rx_queue, K_NO_WAIT))) {
		net_buf_unref(buf);
	}
#endif
}
...
{
	k_fifo_cancel_wait(&bt_dev.rx_queue);
...
int bt_enable(bt_ready_cb_t cb)
{
	...
	irq_lock_init();
#endif /* IS_ENABLED(ZEPHYR_IRQ_LOCK_BY_MUTEX) */
...
#if IS_ENABLED(XRADIO)
#if defined(CONFIG_BT_VAR_MEM_DYNC_ALLOC)
	bt_host_variable_mem_alloc();
#endif

	/* Warning! This function should be a singleton. Error while bt_enable twice.  */
	extern int k_sys_work_q_init(struct device *dev);
	k_sys_work_q_init(NULL);

#if defined(CONFIG_BLEHOST_Z_ITERABLE_SECTION)
	init_mem_slab_module();
#endif
#endif /* IS_ENABLED(XRADIO) */
...
#if defined(CONFIG_BT_VAR_MEM_DYNC_ALLOC)
static void bt_dynamic_mem_alloc(void)
{
	keys_dynamic_mem_alloc();
#if defined(CONFIG_BT_EXT_ADV)
	adv_ext_dynamic_mem_alloc();
#endif
}

static void bt_dynamic_mem_free(void)
{
	keys_dynamic_mem_free();
#if defined(CONFIG_BT_EXT_ADV)
	adv_ext_dynamic_mem_free();
#endif
}

void bt_host_variable_mem_alloc(void)
{
	net_buf_pool_data_mem_alloc();

	bt_dynamic_mem_alloc();
}

void bt_host_variable_mem_free(void)
{
	net_buf_pool_data_mem_free();
...
#if defined(CONFIG_BT_DEINIT)
static void bt_dev_deinit(void)
{
	/* bt_dev.drv->close() should be executed after bt_host_deinit() */
	if (bt_dev.drv)
		bt_dev.drv->close();

	memset(&bt_dev, 0, sizeof(struct bt_dev));
	bt_dev.init = (struct k_work)Z_WORK_INITIALIZER(init_work);
#if !defined(CONFIG_BT_WAIT_NOP)
	bt_dev.ncmd_sem 	 = (struct k_sem)Z_SEM_INITIALIZER(bt_dev.ncmd_sem, 1, 1);
#else
	bt_dev.ncmd_sem 	 = (struct k_sem)Z_SEM_INITIALIZER(bt_dev.ncmd_sem, 0, 1);
#endif
	bt_dev.cmd_tx_queue  = (struct k_fifo)Z_FIFO_INITIALIZER(bt_dev.cmd_tx_queue);
#if !defined(CONFIG_BT_RECV_IS_RX_THREAD)
	bt_dev.rx_queue 	 = (struct k_fifo)Z_FIFO_INITIALIZER(bt_dev.rx_queue);
#endif
}

void bt_conn_deinit_disconn(struct bt_conn *conn, uint8_t reason)
{
	if (k_sem_valid(&deinit_disconn_sem) && (conn == deinit_disconn_conn)) {
		k_sem_give(&deinit_disconn_sem);
	}
}

static struct bt_conn_cb conn_deinit_disconn_callbacks = {
	.disconnected = bt_conn_deinit_disconn,
};

void conn_disconnect_all(struct bt_conn *conn, void *user_data)
{
	int err;

	k_sem_init(&deinit_disconn_sem, 0, 1);

	//traverse all the conn and disconnect the connection if connected
	if (conn->state == BT_CONN_CONNECTED) {
		err = bt_conn_disconnect(conn, BT_HCI_ERR_REMOTE_USER_TERM_CONN);
		if (err) {
			BT_ERR("Disconnection failed (err %d)", err);
		}

		deinit_disconn_conn = conn;
		k_sem_take(&deinit_disconn_sem, 1000);
	} else if (conn->state == BT_CONN_DISCONNECT ||
	...
	if (conn->state != BT_CONN_DISCONNECTED) {
		...
		while (atomic_get(&conn->ref) != 0) {
			bt_conn_unref(conn);
		}
	}

	k_sem_delete(&deinit_disconn_sem);
}
...
static int bt_service_stop(void)
{
	...
	//if device is initing, stop deinit procedure
	if (!atomic_test_bit(bt_dev.flags, BT_DEV_ENABLE) ||
			!atomic_test_bit(bt_dev.flags, BT_DEV_READY)) {
		BT_ERR("BLE Host is not ready");
		return -ENODEV;
	}

	//if ble is advertising, then stop advertise
	if (atomic_test_bit(adv->flags, BT_ADV_ENABLED)) {
		err = bt_le_adv_stop();
		if (err < 0) {
			BT_ERR("Failed to stop advertise (ERR %d)", err);
			return -ENOEXEC;
		} else {
	...
	//if ble is scanning, then stop scan
	if (atomic_test_bit(bt_dev.flags, BT_DEV_SCANNING)) {
		err= bt_le_scan_stop();
		if (err) {
			...
			BT_DBG("Scanning stopped");
		}
	}

	//if ble is connecting, then wait for connect complete
	while (atomic_test_bit(bt_dev.flags, BT_DEV_INITIATING)) {
		k_sleep(3);
	}

	//disconnect all connections
	bt_conn_cb_register(&conn_deinit_disconn_callbacks);
	bt_conn_foreach(BT_CONN_TYPE_ALL, conn_disconnect_all, NULL);
	bt_conn_cb_unregister(&conn_deinit_disconn_callbacks);
...
#if IS_ENABLED(CONFIG_BT_BREDR)
	//if bt is scan, then stop
	if (atomic_test_bit(bt_dev.flags, BT_DEV_ISCAN)) {
	   err = bt_br_set_discoverable(false);
	   if (err) {
			BT_ERR("Failed to stop bt iscan (ERR %d)", err);
			return -ENOEXEC;
		} else {
			BT_DBG("BT IScanning Stopped");
		}
	}
	...
	if (atomic_test_bit(bt_dev.flags, BT_DEV_PSCAN)) {
	   ...
	   if (err) {
			BT_ERR("Failed to stop bt pscan (ERR %d)", err);
			return -ENOEXEC;
		} else {
			BT_DBG("BT PScanning Stopped");
		}
	}

	//if bt is inquiry, then stop
	if (atomic_test_bit(bt_dev.flags, BT_DEV_INQUIRY)) {
		err = bt_br_discovery_stop();
		if (err) {
			BT_ERR("Failed to stop bt inquiry (ERR %d)", err);
			return -ENOEXEC;
		} else {
			BT_DBG("BT Inquiry Stopped");
		}
	}
#endif
...
int thread_disable(void)
{
	int err;

	//ecc thread exit
	err = bt_hci_ecc_deinit();
	if (err) {
		return err;
	}

	//tx thread exit
	err = bt_hci_tx_thread_deinit();
	if (err) {
		return err;
	}

	//rx thread exit
	err = bt_hci_rx_thread_deinit();
	if (err) {
		return err;
	}
...
int common_deinit(void)
{
...
#if IS_ENABLED(XRADIO)
#if defined(CONFIG_BLEHOST_Z_ITERABLE_SECTION)
	deinit_mem_slab_module();
#endif

	/* Warning! This function should be a singleton. Error while bt_enable twice.  */
	extern int k_sys_work_q_deinit(struct device *dev);
	k_sys_work_q_deinit(NULL);
#endif /* IS_ENABLED(XRADIO) */

	if (!atomic_test_and_clear_bit(bt_dev.flags, BT_DEV_ENABLE)) {
		return -EALREADY;
	}

	NET_BUF_POOL_FIXED_DEINIT(&hci_cmd_pool, CONFIG_BT_HCI_CMD_COUNT,
		CMD_BUF_SIZE, NULL);
	...
	sys_slist_init(&scan_cbs);

#if defined(CONFIG_BT_EXT_ADV)
#if !defined(CONFIG_BT_VAR_MEM_DYNC_ALLOC)
	memset(&adv_pool, 0, sizeof(struct bt_le_ext_adv) * ARRAY_SIZE(adv_pool));
#endif
#endif
	...
	/* make sure nowhere use the irq lock by irq_lock() or irq_unlock(), before
	   irq_lock_deinit() be executed */
#if (IS_ENABLED(XRADIO)) && (IS_ENABLED(ZEPHYR_IRQ_LOCK_BY_MUTEX))
	irq_lock_deinit();
#endif /* IS_ENABLED(ZEPHYR_IRQ_LOCK_BY_MUTEX) */

	memset(&cmd_data, 0, sizeof(struct cmd_data) * ARRAY_SIZE(cmd_data));

	return 0;
}

static void le_deinit(void)
{
	k_sem_delete(&bt_dev.le.pkts);
	k_sem_delete(&bt_dev.le.acl_pkts);
#if defined(CONFIG_BT_ISO)
	k_sem_delete(&bt_dev.le.iso_pkts);
#endif

#if defined(CONFIG_BT_OBSERVER)
	k_mutex_delete(&scan_lock);
#endif
	k_mutex_delete(&adv_lock);

#ifdef CONFIG_BT_XR_IMPL
	k_mutex_delete(&rand_addr_lock);
#endif

	prng_reset();
}

static int bt_host_disable(void)
{
	int err;

	//clear BT_DEV_READY bit to avoid new command generate
...
static int bt_host_deinit(void)
{
	int err;

	//stop the service which is running
	err = bt_service_stop();
	if (err) {
		BT_ERR("Stop BLE Service Failed (ERR %d)", err);
		return err;
	}

	//disable bt host stack
	err = bt_host_disable();
	if (err) {
		BT_ERR("BLE Host Disable Failed (ERR %d)", err);
		return err;
	}
...
int bt_disable(void)
{
	...
	err = bt_host_deinit();
	if (err) {
		BT_ERR("BLE Host Deinit Failed (ERR %d)", err);
		return err;
	}

#if defined(CONFIG_BT_VAR_MEM_DYNC_ALLOC)
	bt_host_variable_mem_free();
#endif

#if 0
	/*ble controller deinit*/
	ble_ctrl_deinit();
#endif
...
/**
 * @brief get device local random address.
 *
 * @param *addr: local random address.
 * @return Zero on success or error code otherwise
 */
int bt_get_rand_addr(bt_addr_le_t *addr)
{
	if (addr == NULL) {
		return -EINVAL;
	}
	memcpy(addr, &bt_dev.random_addr, sizeof(*addr));
	return 0;
}
...
#if XRADIO
	if (atomic_test_bit(adv->flags, BT_ADV_PAUSED)) {
...
void bt_le_adv_resume(void)
{
	...
	if (!(atomic_test_bit(adv->flags, BT_ADV_PERSIST) &&
	      !atomic_test_bit(adv->flags, BT_ADV_ENABLED)
#if XRADIO
	      && !atomic_test_bit(adv->flags, BT_ADV_PAUSED)
#endif
	      )) {
	    k_mutex_unlock(&adv_lock);
		return;
