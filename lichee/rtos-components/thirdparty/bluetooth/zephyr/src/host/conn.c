...
#else
static struct bt_conn *acl_conns;
#endif
...
/* to avoid race condition between work_q_thread and hci_rx_thread.
 * detail: if work_q_thread is disruptted when ready to handle cb,
 * hci_rx_thread will ignore next att req packet since cb unhandled
 */
static struct k_mutex tx_notify_lock;
#endif
...
#ifdef CONFIG_BT_XR_IMPL
		k_mutex_lock(&tx_notify_lock, K_FOREVER);
#endif
		key = irq_lock();
		if (sys_slist_is_empty(&conn->tx_complete)) {
			irq_unlock(key);
#ifdef CONFIG_BT_XR_IMPL
			k_mutex_unlock(&tx_notify_lock);
#endif
			break;
		}
...
#if defined(CONFIG_BT_VAR_MEM_DYNC_ALLOC)
	conn = bt_conn_new(acl_conns, CONFIG_BT_MAX_CONN);
#else
...
void bt_conn_cb_unregister(struct bt_conn_cb *cb)
{
	...
	for (tmp_cb = callback_list; tmp_cb; tmp_cb = tmp_cb->_next) {
		if (tmp_cb->_next == cb) {
			tmp_cb->_next = cb->_next;
			return ;
		}
	}

	BT_ERR("conn cb unregister error");
}
#endif
...
static struct bt_conn_tx *conn_tx_alloc(void)
{
	 ...
	 */
	if (k_current_get() == (void *)k_sys_work_q.thread.task.handle) {
...
void bt_conn_set_state(struct bt_conn *conn, bt_conn_state_t state)
{
	...
	case BT_CONN_CONNECTED:
		...
		k_poll_signal_raise(&conn_change, 0); /* same as k_poll_signal */
	...
	case BT_CONN_DISCONNECT:
#if XRADIO
		/* bt_conn_set_state(conn, BT_CONN_DISCONNECT) in conn_disconnect()
		 * might be executed after BT_HCI_EVT_DISCONN_COMPLETE events, which
		 * cause a BT_CONN_DISCONNECT_COMPLETE -> BT_CONN_DISCONNECT but
		 * BT_CONN_DISCONNECT -> BT_CONN_DISCONNECTED failed.
		 */
		if (old_state == BT_CONN_DISCONNECT_COMPLETE || old_state == BT_CONN_DISCONNECTED) {
			conn->state = old_state;
			BT_WARN("Disconn event came before set BT_CONN_DISCONNECT state");
		}
#endif
		break;
	case BT_CONN_DISCONNECT_COMPLETE:
...
#if defined(CONFIG_BT_VAR_MEM_DYNC_ALLOC)
	conn = conn_lookup_handle(acl_conns, CONFIG_BT_MAX_CONN, handle);
#else
...
#if XRADIO
		/*
		 * should setup the create_param, otherwise it will cause connection parameters
		 * scan interval and scan window not initialized.
		 */
		struct bt_conn_le_create_param create_param;
		create_param.timeout = (MSEC_PER_SEC / 10) * CONFIG_BT_CREATE_CONN_TIMEOUT;
...
#if defined(CONFIG_BT_VAR_MEM_DYNC_ALLOC)
		__ASSERT(0 <= index && index < CONFIG_BT_MAX_CONN,
#else
...
#if defined(CONFIG_BT_VAR_MEM_DYNC_ALLOC)
	acl_conns = (struct bt_conn *)k_malloc(sizeof(struct bt_conn) * CONFIG_BT_MAX_CONN);
	if (acl_conns == NULL) {
		BT_ERR("Alloc acl_conns error");
		return -ENOMEM;
	}
	memset(acl_conns, 0, sizeof(struct bt_conn) * CONFIG_BT_MAX_CONN);
#endif
...
int bt_conn_deinit(void)
{
	...
	memset(&conn_tx, 0, sizeof(struct bt_conn_tx) * ARRAY_SIZE(conn_tx));

	free_tx = (struct k_fifo)Z_FIFO_INITIALIZER(free_tx);
...
void bt_conn_mem_deinit(void)
{
	NET_BUF_POOL_FIXED_DEINIT(&acl_tx_pool, CONFIG_BT_L2CAP_TX_BUF_COUNT,
			BT_L2CAP_BUF_SIZE(CONFIG_BT_L2CAP_TX_MTU), NULL);

	NET_BUF_POOL_FIXED_DEINIT(&frag_pool, CONFIG_BT_L2CAP_TX_FRAG_COUNT,
			FRAG_SIZE, NULL);
	...
	conn_change = (struct k_poll_signal)K_POLL_SIGNAL_INITIALIZER(conn_change);

#if !defined(CONFIG_BT_VAR_MEM_DYNC_ALLOC)
	memset(&acl_conns, 0, sizeof(struct bt_conn) * ARRAY_SIZE(acl_conns));
#endif
}
#endif
