...
static bool hal_open(const hci_hal_callbacks_t *upper_callbacks, void *task_thread)
{
    ...
    if (vendor && vendor->open)
        vendor->open(NULL, NULL);
    else
        HCI_TRACE_ERROR("vendor not exist!\n");
#else
    #error "should enable CONFIG_BT_DRIVERS_LIB!"
#endif

#if 0//ToDo...
    vendor->send_async_command(BT_VND_OP_SCO_CFG, VENDOR_SCO_PATH_HCI | VENDOR_SCO_CODEC_CSVD);
#endif
...
#if 0
static int32_t read_data(uint8_t *buffer, int32_t max_size)
{
#define VENDOR_RX_TIMEOUT_MS (-1)

    return vendor->receive(buffer, max_size, VENDOR_RX_TIMEOUT_MS);
}
#endif
...
/**
 * In the original function transmit_data implementation,
 * while executing vendor->transmit(..), may have received the
 * command complete event, which will free the command buff memmory.
 * afrer that, to execute "*(data) = previous_byte;" may cause step
 * on memory.
 */
                              ...
                              uint8_t *data, uint16_t length)
{
    ...
    // TX Data to target
    vendor->transmit(type, data, length);
                              ...
                              uint8_t *data, uint16_t length)
{
    ...
    //BTTRC_DUMP_BUFFER(false, data, length, type);
...
task_post_status_t hci_hal_h4_task_post(uint32_t timeout)
{
...
static void hci_packet_complete(BT_HDR *packet){
    ...
    if (type == DATA_TYPE_ACL/* || type == DATA_TYPE_SCO*/) {
        ...
        if (xr_vhci_host_check_send_available()){
...
int hci_hal_h4_recv(const uint8_t *data, uint16_t len)
{
