...
tL2C_LCB *l2cu_allocate_lcb (BD_ADDR p_bd_addr, BOOLEAN is_bonding, tBT_TRANSPORT transport)
{
    ...
    for (xx = 0; xx < MAX_L2CAP_LINKS; xx++, p_lcb++) {
        if (!p_lcb->in_use) {
            ...
            p_lcb->timer_entry.param = (TIMER_PARAM_TYPE)(uintptr_t)p_lcb;
            p_lcb->info_timer_entry.param = (TIMER_PARAM_TYPE)(uintptr_t)p_lcb;
            p_lcb->upda_con_timer.param = (TIMER_PARAM_TYPE)(uintptr_t)p_lcb;
...
tL2C_CCB *l2cu_allocate_ccb (tL2C_LCB *p_lcb, UINT16 cid)
{
    ...
    p_ccb->fcrb.ack_timer.param  = (TIMER_PARAM_TYPE)(uintptr_t)p_ccb;

    btu_free_quick_timer(&p_ccb->fcrb.mon_retrans_timer);
    memset(&p_ccb->fcrb.mon_retrans_timer, 0, sizeof(TIMER_LIST_ENT));
    p_ccb->fcrb.mon_retrans_timer.param  = (TIMER_PARAM_TYPE)(uintptr_t)p_ccb;
...
// btla-specific --
    ...
    p_ccb->timer_entry.param = (TIMER_PARAM_TYPE)(uintptr_t)p_ccb;
...
static tL2C_CCB *l2cu_get_next_channel_in_rr(tL2C_LCB *p_lcb)
{
    ...
    for ( i = 0; (i < L2CAP_NUM_CHNL_PRIORITY) && (!p_serve_ccb); i++ ) {
        ...
        for ( j = 0; (j < p_lcb->rr_serv[p_lcb->rr_pri].num_ccb) && (!p_serve_ccb); j++) {
            ...
            L2CAP_TRACE_DEBUG("RR scan pri=%d, lcid=0x%04x, q_cout=%zu",
        ...
        if ( p_ccb->cong_sent ) {
            ...
            if (q_count <= (p_ccb->buff_quota / 2)) {
                ...
                if (p_ccb->p_rcb && p_ccb->p_rcb->api.pL2CA_CongestionStatus_Cb) {
                    L2CAP_TRACE_DEBUG ("L2CAP - Calling CongestionStatus_Cb (FALSE), CID: 0x%04x  xmit_hold_q.count: %zu  buff_quota: %u",
                    ...
                    if ( p_ccb->p_rcb->ucd.cb_info.pL2CA_UCD_Congestion_Status_Cb ) {
                        L2CAP_TRACE_DEBUG ("L2CAP - Calling UCD CongestionStatus_Cb (FALSE), SecPendingQ:%zu,XmitQ:%zu,Quota:%u",
            ...
            if (q_count > p_ccb->buff_quota || (p_lcb && (p_lcb->link_xmit_data_q) && (list_length(p_lcb->link_xmit_data_q) + q_count) > p_ccb->buff_quota)) {
                ...
                if (p_ccb->p_rcb && p_ccb->p_rcb->api.pL2CA_CongestionStatus_Cb) {
                    L2CAP_TRACE_DEBUG ("L2CAP - Calling CongestionStatus_Cb (TRUE),CID:0x%04x,XmitQ:%zu,Quota:%u",
                                       p_ccb->local_cid, q_count, p_ccb->buff_quota);
                    ...
                    if ( p_ccb->p_rcb->ucd.cb_info.pL2CA_UCD_Congestion_Status_Cb ) {
                        L2CAP_TRACE_DEBUG ("L2CAP - Calling UCD CongestionStatus_Cb (TRUE), SecPendingQ:%zu,XmitQ:%zu,Quota:%u",
                                          fixed_queue_length(p_ccb->p_lcb->ucd_out_sec_pending_q),
                                          fixed_queue_length(p_ccb->xmit_hold_q),
                                          p_ccb->buff_quota);
