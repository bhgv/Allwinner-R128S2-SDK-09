    ...
    if (pkt_type != L2CAP_PKT_CONTINUE) {
        ...
        if ((p_lcb = l2cu_find_lcb_by_handle (handle)) == NULL) {
            ...
            if ((p_msg->layer_specific == 0) && (rcv_cid == L2CAP_SIGNALLING_CID)
                ...
                L2CAP_TRACE_DEBUG ("L2CAP - holding ACL for unknown handle:%d ls:%d"
                                     "  cid:%d opcode:%d cur count:%zu", handle, p_msg->layer_specific,
...
#ifdef CONFIG_BT_DUAL_HOST
                L2CAP_TRACE_DEBUG ("L2CAP - rcvd ACL for unknown handle:%d ls:%d cid:%d"
                                   " opcode:%d cur count:%zu", handle, p_msg->layer_specific, rcv_cid,
...
void l2c_process_timeout (TIMER_LIST_ENT *p_tle)
{
    ...
    case BTU_TTYPE_L2CAP_LINK:
        l2c_link_timeout ((tL2C_LCB *)(uintptr_t)p_tle->param);
        break;
#if (CLASSIC_BT_INCLUDED == TRUE)
    case BTU_TTYPE_L2CAP_CHNL:
        l2c_csm_execute (((tL2C_CCB *)(uintptr_t)p_tle->param), L2CEVT_TIMEOUT, NULL);
        break;

    case BTU_TTYPE_L2CAP_FCR_ACK:
        l2c_csm_execute (((tL2C_CCB *)(uintptr_t)p_tle->param), L2CEVT_ACK_TIMEOUT, NULL);
        break;
#endif  ///CLASSIC_BT_INCLUDED == TRUE
    ...
    case BTU_TTYPE_L2CAP_INFO:
        l2c_info_timeout((tL2C_LCB *)(uintptr_t)p_tle->param);
        break;
...
UINT8 l2c_data_write (UINT16 cid, BT_HDR *p_data, UINT16 flags)
{
    ...
    if (p_ccb->cong_sent) {
        L2CAP_TRACE_DEBUG ("L2CAP - CID: 0x%04x cannot send, already congested  xmit_hold_q.count: %zu  buff_quota: %u",
                           p_ccb->local_cid,
