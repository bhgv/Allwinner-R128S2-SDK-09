...
void *iterate(void *pres) {
	...
	for (i=0; i<iterations; i++) {
		...
		res->crc=coremark_crcu16(crc,res->crc);
		crc=core_bench_list(res,-1);
		res->crc=coremark_crcu16(crc,res->crc);
...
#error "Please define a way to initialize a memory block."
	...
	seedcrc=coremark_crc16(results[0].seed1,seedcrc);
	seedcrc=coremark_crc16(results[0].seed2,seedcrc);
	seedcrc=coremark_crc16(results[0].seed3,seedcrc);
	seedcrc=coremark_crc16(results[0].size,seedcrc);
...
int cmd_coremark(int argc, char ** argv)
{
	return coremark_main();
}

FINSH_FUNCTION_EXPORT_CMD(cmd_coremark, coremark, benchmark);
