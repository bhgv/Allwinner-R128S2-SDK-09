...
void lv_spilcd_get_sizes(uint32_t *width, uint32_t *height)
{
    *width = bsp_disp_get_screen_width(g_screen_index);
    *height = bsp_disp_get_screen_height(g_screen_index);
}

void lv_spilcd_init(uint32_t bpp, long int smem_len, char **mem_start)
{
    struct fb_info info;
...
void lv_spilcd_exit(void)
{
    sunxifb_mem_free((void**) &fbsmem_start, "spilcd");
}

int lv_spilcd_pan_display(uint32_t yoffset)
{
    struct fb_info info;
    memset(&info, 0, sizeof(struct fb_info));
