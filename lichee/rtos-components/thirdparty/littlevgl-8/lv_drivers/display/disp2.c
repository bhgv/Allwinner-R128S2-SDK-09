...
static int disp2_set_layer(struct set_layer_cfg *cfg, uint32_t yoffset)
{
    ...
    return lv_disp_cfg_layer(g_screen_index, cfg);

}

void lv_disp2_get_sizes(uint32_t *width, uint32_t *height) {
    unsigned long arg[6];
...
void lv_disp2_init(uint32_t bpp, long int smem_len, char **mem_start)
{
    ...
    lv_disp_clear_layer(g_screen_index, chn, layer);
...
int lv_disp2_pan_display(uint32_t yoffset)
{
    struct set_layer_cfg cfg;
    unsigned long arg[6];
    memset(&cfg, 0, sizeof(struct set_layer_cfg));
